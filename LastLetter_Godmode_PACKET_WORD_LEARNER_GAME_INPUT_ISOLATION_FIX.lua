...
local function IsWordHelperTextBox(box)
    return box
        and box:IsA("TextBox")
        and ScreenGui
...
local function ReleaseWordHelperTextFocus()
    local focused = UserInputService:GetFocusedTextBox()
    if IsWordHelperTextBox(focused) then
        pcall(function()
...
local function ClearActualGameInput(extraBackspaces)
    local target = GetGameTextBox()
    if target then
        target.Text = ""
        return
    end

    ReleaseWordHelperTextFocus()
    ...
    local success, err = pcall(function()
        if isCorrection then
            ...
            if not accepted then
                ...
                ClearActualGameInput(#targetWord + 5)
