...
local function Backspace(count)
    ...
    -- Only edit an actual Last Letter TextBox directly.
    if focused and inGame and focused:IsDescendantOf(inGame) and focused.TextEditable then
        local currentText = focused.Text
        focused.Text = currentText:sub(1, -count - 1)
        lastKey = nil
        return
    end

    -- Never let cleanup/backspaces land in WordHelper's own TextBoxes.
...
env.WordHelperIsOwnTextBox = function(box)
    return box
        and box:IsA("TextBox")
        and ScreenGui
        and box:IsDescendantOf(ScreenGui)
end

env.WordHelperGetGameTextBox = function()
    local player = Players.LocalPlayer
    local gui = player and player:FindFirstChild("PlayerGui")
    local inGame = gui and gui:FindFirstChild("InGame")
    if inGame then
        ...
        if frame then
            for _, c in ipairs(frame:GetDescendants()) do
                if c:IsA("TextBox") and c.Visible and c.TextEditable then
                    return c
                end
            end
        end
    ...
    -- Never fall back to WordHelper's own search/input fields.
    return nil
end

env.WordHelperReleaseOwnTextFocus = function()
    local focused = UserInputService:GetFocusedTextBox()
    if env.WordHelperIsOwnTextBox(focused) then
        pcall(function()
...
env.WordHelperClearActualGameInput = function(extraBackspaces)
    local target = env.WordHelperGetGameTextBox()
    if target then
        target.Text = ""
        return
    end

    env.WordHelperReleaseOwnTextFocus()
    local count = math.max(1, tonumber(extraBackspaces) or 1)
    for _ = 1, count do
        pcall(function()
...
local function SmartType(targetWord, currentDetected, isCorrection, bypassTurn)
    ...
    local targetBox = env.WordHelperGetGameTextBox()

    -- Auto Play must never type into WordHelper's own UI.
    env.WordHelperReleaseOwnTextFocus()
    ...
    local success, err = pcall(function()
        if isCorrection then
            ...
            local finalCheck = env.WordHelperGetGameTextBox()
            if not riskyMistakes then
                task.wait(0.1)
                finalCheck = env.WordHelperGetGameTextBox()
            ...
            if not accepted then
                ...
                env.WordHelperClearActualGameInput(#targetWord + 5)
            ...
            if not accepted then
                ...
                local postCheck = env.WordHelperGetGameTextBox()
