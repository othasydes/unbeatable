-- V20.8.4: zero-dwell UsedWords acceptance capture for ultra-fast replies
...
runConn = RunService.RenderStepped:Connect(function()
    local success, err = pcall(function()
        ...
        if isVisible then
            ...
            -- V20.8.4: commit the latest complete dictionary candidate immediately
            -- when ownership/Type advances. The previous 80 ms dwell gate could miss
            -- rapid spam exchanges even though the accepted word was observed.
            if typeText ~= env.WordHelperUsedTracker.lastObservedTypeText
                and env.WordHelperUsedTracker.observedWord ~= "" then
                env.WordHelperUsedTracker.RecordObservedUsedWord(env.WordHelperUsedTracker.observedWord)
                env.WordHelperUsedTracker.observedWord = ""
            end
        end
