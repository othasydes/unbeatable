...
-- Keep performance-worker state in the existing environment table so these values
-- do not consume scarce main-chunk Luau local registers.
env = env or ((getgenv and getgenv()) or _G)
env.WordHelperProRefresh = env.WordHelperProRefresh or {Generation = 0, Running = false}
...
env.WordHelperUnbeatable.SelectTopProCandidates = function(candidateList)
    ...
    if reverse then
        ...
        for candidateIndex,w in ipairs(originalCandidateList) do
            if candidateIndex % 160 == 0 then task.wait() end
...
UpdateList = function(detectedText, requiredLetter)
        ...
        if bucket then
            ...
            if useBinary and #prefix > 0 then
                ...
                if startIndex ~= -1 then
                    ...
                    for i = startIndex, #bucket do
                        if sortMode == "Pro Unbeatable" and (i - startIndex) % 320 == 319 then task.wait() end
                        local w = bucket[i]
    ...
    if sortMode == "Pro Unbeatable" and env.WordHelperProRefresh.Running
        and (detectedText ~= lastDetected or requiredLetter ~= lastRequiredLetter) then
        return
    end
...
runConn = RunService.RenderStepped:Connect(function()
    local success, err = pcall(function()
        ...
        if listUpdatePending and (now - lastInputTime > LIST_DEBOUNCE) then
            listUpdatePending = false
            if sortMode == "Pro Unbeatable" then
                env.WordHelperProRefresh.Generation = env.WordHelperProRefresh.Generation + 1
                if not env.WordHelperProRefresh.Running then
                    env.WordHelperProRefresh.Running = true
                    task.spawn(function()
                        task.wait()
                        while not unloaded and sortMode == "Pro Unbeatable" do
                            local generation = env.WordHelperProRefresh.Generation
                            UpdateList(lastDetected, lastRequiredLetter)
                            if generation == env.WordHelperProRefresh.Generation then break end
                            task.wait()
                        end
                        env.WordHelperProRefresh.Running = false
                    end)
                end
            else
                UpdateList(lastDetected, lastRequiredLetter)
