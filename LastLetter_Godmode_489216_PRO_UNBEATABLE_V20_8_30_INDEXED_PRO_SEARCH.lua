-- V20.8.30: indexed/cached full-coverage Pro discovery; V20.8.29 search quality preserved
...
env.WordHelperUnbeatable.SelectTopProCandidates = function(candidateList)
    ...
    -- V20.8.30 NORMAL PRO FULL-COVERAGE CACHED DISCOVERY.
    ...
    local discoveryKey = table.concat({
        "s" .. tostring(stage),
        tostring(#originalCandidateList),
        firstCandidate,
        lastCandidate,
    ...
    if cachedDiscovery then
        ...
        -- Exact V20.8.29 full-pool discovery. Periodic yielding keeps a cache miss
        -- responsive without reducing coverage or changing which returns can win.
        for wi, w in ipairs(originalCandidateList) do
            if w:sub(-1) == "x" and easierTrigger(w, xBest) then
                xBest = w
            end

            local chosenPrefix = nil
            local chosenNonSelf = 999999
            local chosenSelf = false

            for suffixLen = math.min(stage, #w), 1, -1 do
                local prefix = w:sub(-suffixLen)
                local startsWithLetter = prefix:match("^[a-z]") ~= nil
                local hasPunctuation = prefix:find("'", 1, true) ~= nil
                    or prefix:find("-", 1, true) ~= nil
                local punctuationLengthAllowed = (not hasPunctuation) or suffixLen >= 3
                ...
                if startsWithLetter and punctuationLengthAllowed then
                    ...
                    if otherReplies >= 1 then
                        chosenPrefix = prefix
                        chosenNonSelf = otherReplies
                        chosenSelf = availableSelf
                        break
                    end
                end
            end
        ...
        local discovered = {}
        for _, row in pairs(returnBest) do
            discovered[#discovered + 1] = row
        end
        ...
        if stage == 2 then
            for _, row in ipairs(discovered) do
                ...
                elseif #p == 2 and env.WordHelperUnbeatable.ProGodmodeTwoLetterTraps[p]
                    ...
                    regularSeen[p] = true
                    regularStageTwo[#regularStageTwo + 1] = row.Word
                end
            end
        ...
        -- Small LRU-ish cache. Keep recent structural discoveries only.
        env.WordHelperUnbeatable.ProDiscoveryCache[discoveryKey] = {
            Shortlist = shortlist,
            Spicy = spicyStageTwo,
            Regular = regularStageTwo
        }
