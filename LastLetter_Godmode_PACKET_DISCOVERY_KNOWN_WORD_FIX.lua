...
env.WordHelperDiscovery.IsKnownWord = function(word)
    ...
    -- Discovery must never classify an existing dictionary word as unknown.
    -- Use an exact scan rather than relying on bucket sort state. This runs only
    -- when an accepted packet candidate is being reviewed, so it remains cheap.
