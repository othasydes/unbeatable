...
env.WordHelperDiscovery.ObserveFrame = function(detectedWord, censored, typeText, now, frame)
    ...
    -- IMPORTANT: Last Letter truncates long typed words visually with "..".
    -- Never strip those dots and treat the remaining suffix as a complete word.
    -- Instead reconstruct the hidden prefix from the immediately preceding full
    -- candidate. If reconstruction is not trustworthy, skip discovery entirely.
    local raw = tostring(detectedWord or ""):lower():gsub("%s+", "")
    local visible = raw:gsub("[^a-z]", "")
    local hasTruncation = raw:find("..", 1, true) ~= nil or raw:find("…", 1, true) ~= nil

    if #visible >= 2 then
        if not hasTruncation then
            -- While the full word still fits on screen, this is authoritative.
            ...
            env.WordHelperDiscovery.CurrentCandidateType = tostring(typeText or "")
        else
            ...
            if #full >= 2 then
                -- If the visible suffix is already the end of our full cache,
                -- nothing new needs to be appended yet.
                if #visible <= #full and full:sub(-#visible) == visible then
                    reconstructed = full
                else
                    -- Find the largest overlap between the end of the previous full
                    -- word and the beginning of the currently visible suffix.
            ...
            if reconstructed and #reconstructed >= #visible then
                ...
                -- A truncated suffix by itself is NOT enough evidence to learn.
    ...
    if wpmText == env.WordHelperDiscovery.LastWPMText then return end
    env.WordHelperDiscovery.LastWPMText = wpmText
    ...
    -- For a long/truncated word, only accept the reconstructed full candidate.
    if env.WordHelperDiscovery.CandidateTruncated
        and env.WordHelperDiscovery.ReconstructionValid ~= true then
        return
    end

    if #candidate >= 2 and (now - candidateAt) <= 0.75 then
        env.WordHelperDiscovery.ConsiderAccepted(candidate, "WPM")
    end
end
