...
local proRefreshGeneration = 0
local proRefreshRunning = false
...
UpdateList = function(detectedText, requiredLetter)
    ...
    if sortMode == "Pro Unbeatable" and proRefreshRunning
...
runConn = RunService.RenderStepped:Connect(function()
    local success, err = pcall(function()
        ...
        if listUpdatePending and (now - lastInputTime > LIST_DEBOUNCE) then
            listUpdatePending = false
            if sortMode == "Pro Unbeatable" then
                proRefreshGeneration = proRefreshGeneration + 1
                if not proRefreshRunning then
                    proRefreshRunning = true
                    task.spawn(function()
                        task.wait()
                        while not unloaded and sortMode == "Pro Unbeatable" do
                            local generation = proRefreshGeneration
                            UpdateList(lastDetected, lastRequiredLetter)
                            if generation == proRefreshGeneration then break end
                            task.wait()
                        end
                        proRefreshRunning = false
                    end)
                end
            else
                UpdateList(lastDetected, lastRequiredLetter)
