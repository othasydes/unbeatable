-- V20.8.11: sliding CurrentWord reconstruction + SERVER-ONLY AnswerResults UsedWords tracking
...
env.WordHelperLongWord.MaybeLock = function(word, typedPrefix)
    ...
    typedPrefix = tostring(typedPrefix or ""):lower():gsub("[%s%c]+", "")

    -- Only intervene for genuinely long entries and only after enough characters
    -- have been typed to make an accidental lock extremely unlikely.
    if #word <= 12 or #typedPrefix < 6 then return end
    if word:sub(1, #typedPrefix) ~= typedPrefix then return end

    -- V20.8.9: once a long recommendation is locked, DO NOT replace it just because
    -- sorting produces a different #1 result on a later keystroke. That was the
    -- remaining cause of long entries disappearing around the UI truncation point.
    if env.WordHelperLongWord.Active and env.WordHelperLongWord.Word ~= "" then
        local locked = env.WordHelperLongWord.Word
        if locked:sub(1, #typedPrefix) == typedPrefix then
            env.WordHelperLongWord.Progress = math.max(
                tonumber(env.WordHelperLongWord.Progress or 0) or 0,
                #typedPrefix
            )
            env.WordHelperLongWord.LastVisible = typedPrefix
            return
        end
    end

    env.WordHelperLongWord.Word = word
    env.WordHelperLongWord.Progress = math.max(
        tonumber(env.WordHelperLongWord.Progress or 0) or 0,
        #typedPrefix
    )
    env.WordHelperLongWord.LastVisible = typedPrefix
    env.WordHelperLongWord.Active = true
end

env.WordHelperLongWord.ReconstructSliding = function(previousText, rawText)
    local previous = tostring(previousText or ""):lower():gsub("[^a-z]", "")
    local raw = tostring(rawText or ""):lower()
    local hasOverflowMarker = raw:find("%.%.%.", 1, false) ~= nil
    local visible = raw:gsub("[^a-z]", "")

    if not hasOverflowMarker or previous == "" or visible == "" then
        return visible
    end

    -- The game's long-word UI replaces the letters that have scrolled off the left
    -- side with an ellipsis tile. Stitch the new visible window onto the previous
    -- full prefix by finding the largest suffix/prefix overlap.
    local maxOverlap = math.min(#previous, #visible)
    for overlap = maxOverlap, 1, -1 do
        if previous:sub(#previous - overlap + 1) == visible:sub(1, overlap) then
            return previous .. visible:sub(overlap + 1)
        end
    end

    -- If a frame was caught during a UI rebuild, preserve the last full prefix.
    return previous
end

env.WordHelperLongWord.Resolve = function(visibleText, requiredLetter)
    local state = env.WordHelperLongWord
    local visible = tostring(visibleText or ""):lower():gsub("[%s%c]+", "")
    local required = tostring(requiredLetter or ""):lower():gsub("[%s%c]+", "")

    if not state.Active or state.Word == "" then
        return visible
    end

    local word = state.Word
    local oldProgress = tonumber(state.Progress or 0) or 0

    -- Normal phase: the game is still exposing the full beginning of the entry.
    if visible ~= "" and word:sub(1, #visible) == visible then
        state.Progress = math.max(oldProgress, #visible)
        state.LastVisible = visible
        return word:sub(1, state.Progress)
    end

    -- Long-entry phase: CurrentWord may expose only a moving window/suffix.
    -- Locate that visible window inside the locked recommendation and choose the
    -- earliest occurrence whose END does not move our typing progress backwards.
    if visible ~= "" then
        local searchFrom = 1
        local bestEnd = nil
        while true do
            local s, e = word:find(visible, searchFrom, true)
            if not s then break end
            if e >= oldProgress and (not bestEnd or e < bestEnd) then
                bestEnd = e
            end
            searchFrom = s + 1
        end
        ...
        if bestEnd then
            state.Progress = math.max(oldProgress, bestEnd)
            state.LastVisible = visible
            return word:sub(1, state.Progress)
        end
    end

    -- A fresh prompt/turn means the old long-word lock is no longer relevant.
    if required ~= "" and word:sub(1, #required) ~= required then
        state.Reset()
        return visible
    end

    -- Brief UI rebuilds should not make the recommendation disappear.
    if visible == "" and oldProgress > 0 then
        return word:sub(1, oldProgress)
    end

    -- V20.8.10: CurrentWord can stop mapping cleanly once Last Letter truncates
    -- a long entry. Do not release the recommendation because of that UI limitation.
...
UpdateList = function(detectedText, requiredLetter)
    ...
    -- V20.8.9 STICKY TARGET: preserve the chosen long recommendation itself even if
    -- the game's long CurrentWord UI is rebuilding/truncating while the player types.
    if not manualSearch
        and env.WordHelperLongWord
        and env.WordHelperLongWord.Active
        and env.WordHelperLongWord.Word ~= "" then

        local lockedWord = env.WordHelperLongWord.Word
        if not Blacklist[lockedWord]
            and (not UsedWords[lockedWord] or Config.ShowUsedWords) then

            local alreadyThere = false
            for _, existing in ipairs(exacts) do
                if existing == lockedWord then
                    alreadyThere = true
                    break
                end
            end
            if not alreadyThere then
                table.insert(exacts, 1, lockedWord)
            end
        end
    end
    ...
    -- V20.8.9: while a long target is active and still matches typed progress,
    -- keep it visibly pinned at the top until completion/change of course.
    if not manualSearch
        and env.WordHelperLongWord
        and env.WordHelperLongWord.Active
        and env.WordHelperLongWord.Word ~= "" then
        ...
        for i, w in ipairs(matches) do
            if w == lockedWord then
                foundAt = i
                break
            end
        end
        if foundAt and foundAt > 1 then
            table.remove(matches, foundAt)
            table.insert(matches, 1, lockedWord)
        elseif not foundAt
            and not Blacklist[lockedWord]
            and (not UsedWords[lockedWord] or Config.ShowUsedWords) then
            table.insert(matches, 1, lockedWord)
        end
    end
    ...
    if #matches > 0 and not isBacktracked then
        ...
        if env.WordHelperLongWord and env.WordHelperLongWord.MaybeLock then
            env.WordHelperLongWord.MaybeLock(currentBestMatch, searchPrefix)
        end
    else
