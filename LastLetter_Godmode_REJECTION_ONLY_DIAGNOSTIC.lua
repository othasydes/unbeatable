...
-- Kept separate from Buckets so discovery never depends on search-index state.
...
env.WordHelperBlacklistTracker.RejectionDiagnosticFile = "WordHelper_RejectionDiagnostic.txt"

env.WordHelperBlacklistTracker.LogRejectionDiagnostic = function(stage, word, extra)
    local cleanWord = tostring(word or ""):lower():gsub("[^a-z]", "")
    local line = string.format(
        "[%s] stage=%s word=%s pending=%s pendingAge=%.3f used=%s blacklisted=%s extra=%s",
        os.date("%Y-%m-%d %H:%M:%S"),
        tostring(stage or "?"),
        cleanWord,
        tostring(env.WordHelperBlacklistTracker.PendingWord or ""),
        math.max(0, tick() - (tonumber(env.WordHelperBlacklistTracker.PendingAt or 0) or 0)),
        tostring(UsedWords and UsedWords[cleanWord] == true or false),
        tostring(Blacklist and Blacklist[cleanWord] == true or false),
        tostring(extra or "")
    )
    pcall(function()
        if appendfile then
            appendfile(env.WordHelperBlacklistTracker.RejectionDiagnosticFile, line .. "\n")
        elseif writefile then
            local old = ""
            if isfile and readfile and isfile(env.WordHelperBlacklistTracker.RejectionDiagnosticFile) then
                old = readfile(env.WordHelperBlacklistTracker.RejectionDiagnosticFile) or ""
            end
            writefile(env.WordHelperBlacklistTracker.RejectionDiagnosticFile, old .. line .. "\n")
        end
    end)
    pcall(function() print("[WordHelper Rejection Diagnostic] " .. line) end)
end
...
env.WordHelperBlacklistTracker.Add = function(word, reason)
    word = tostring(word or ""):lower():gsub("[^a-z]", "")
    env.WordHelperBlacklistTracker.LogRejectionDiagnostic("add_called", word, "reason=" .. tostring(reason or ""))
    if #word < 2 or Blacklist[word] then
        env.WordHelperBlacklistTracker.LogRejectionDiagnostic("add_rejected_basic", word, "")
        return false
    end
    ...
    env.WordHelperBlacklistTracker.LogRejectionDiagnostic("add_saved", word, "")
    forceUpdateList = true
    lastDetected = "---"
...
env.WordHelperBlacklistTracker.BeginAttempt = function(word, requiredLetters)
    word = tostring(word or ""):lower():gsub("[^a-z]", "")
    env.WordHelperBlacklistTracker.LogRejectionDiagnostic("begin_attempt", word, "required=" .. tostring(requiredLetters or ""))
    ...
    if #word < 2 or Blacklist[word] or UsedWords[word] then
        env.WordHelperBlacklistTracker.LogRejectionDiagnostic("begin_rejected_basic", word, "")
        return false
    end
    if not env.WordHelperBlacklistTracker.IsKnownWord(word) then
        env.WordHelperBlacklistTracker.LogRejectionDiagnostic("begin_rejected_not_known", word, "")
        return false
    end
    ...
    if not frame then
        env.WordHelperBlacklistTracker.LogRejectionDiagnostic("begin_rejected_no_frame", word, "")
        return false
    end

    local isMyTurn, currentRequired = GetTurnInfo(frame)
    if not isMyTurn then
        env.WordHelperBlacklistTracker.LogRejectionDiagnostic("begin_rejected_not_turn", word, "")
        return false
    end
    ...
    if requiredLetters ~= ""
        ...
        env.WordHelperBlacklistTracker.LogRejectionDiagnostic("begin_rejected_prefix", word, "currentRequired=" .. tostring(currentRequired or ""))
        return false
    end
    ...
    if timerAtSubmit and timerAtSubmit < 1.5 then
        env.WordHelperBlacklistTracker.LogRejectionDiagnostic("begin_rejected_timer", word, "timer=" .. tostring(timerAtSubmit))
        return false
    end
    ...
    env.WordHelperBlacklistTracker.LogRejectionDiagnostic("begin_armed", word, "timer=" .. tostring(timerAtSubmit))
...
env.WordHelperBlacklistTracker.HandleWrongSound = function(source)
    if unloaded then return end
    env.WordHelperBlacklistTracker.LogRejectionDiagnostic("wrong_sound_seen", env.WordHelperBlacklistTracker.PendingWord or "", "source=" .. tostring(source or ""))
    ...
    -- The Wrong sound is global, so only trust it when our Enter handler has
    -- already armed a fresh local submission. BeginAttempt performed all the
    -- safety checks BEFORE submission: my turn, correct starting prefix,
    -- dictionary membership, not already blacklisted/used, and not near timeout.
    local pendingWord =
        tostring(env.WordHelperBlacklistTracker.PendingWord or "")
            :lower():gsub("[^a-z]", "")
    local pendingAt =
    ...
    if not hasFreshPending then
        -- A global Wrong sound with no local armed attempt belongs to somebody
        -- else (or arrived too late), so never blacklist from it.
        return
    end

    -- Do NOT re-check GetTurnInfo/current required letters here. The game may
    -- already have changed the turn UI because the word was rejected.
    local candidate = pendingWord

    -- The pending attempt was checked against UsedWords before Enter. A later
    -- UsedWords flag can be the observer racing the rejection animation, so it
    -- must not veto this confirmed rejection.
    if Blacklist[candidate] then
        env.WordHelperBlacklistTracker.LogRejectionDiagnostic("wrong_already_blacklisted", candidate, "")
        env.WordHelperBlacklistTracker.ClearPending()
        return
    end

    if not env.WordHelperBlacklistTracker.IsKnownWord(candidate) then
        env.WordHelperBlacklistTracker.LogRejectionDiagnostic("wrong_not_known", candidate, "")
        env.WordHelperBlacklistTracker.ClearPending()
        if ShowToast then
            ShowToast("Rejected capture skipped - not in dictionary: " .. candidate, "warning")
        end
        return
    end

    env.WordHelperBlacklistTracker.LogRejectionDiagnostic("wrong_before_add", candidate, "")
    local added = env.WordHelperBlacklistTracker.Add(
        candidate,
        "confirmed local pending attempt + SoundService.Game.Wrong via "
            .. tostring(source or "sound")
    )

    env.WordHelperBlacklistTracker.LogRejectionDiagnostic("wrong_add_result", candidate, "added=" .. tostring(added))
    env.WordHelperBlacklistTracker.ClearPending()

    if added and StatusText then
        StatusText.Text = "Blacklisted from rejection: " .. candidate
        StatusText.TextColor3 = THEME.Warning
    end
end
