...
env.WordHelperBlacklistTracker.Save = function()
    if not writefile then return end
    local list = {}
    for word, blocked in pairs(Blacklist) do
        if blocked then
            table.insert(list, word)
        end
    end
...
env.WordHelperDiscovery.DiagnosticsOnly = true -- F7 accepted-word investigation: do not learn new words
...
env.WordHelperDiscovery.IsKnownWord = function(word)
    ...
    while left <= right do
        ...
        local value = bucket[mid]
        if value == word then return true end
        if value < word then left = mid + 1 else right = mid - 1 end
    end
    return false
end
...
env.WordHelperDiscovery.ConsiderAccepted = function(word, source)
    if env.WordHelperDiscovery.DiagnosticsOnly then
        return false
    end
    word = tostring(word or ""):lower():gsub("[^a-z]", "")
    if #word < 2 or #word > 40 then return false end
    if env.WordHelperDiscovery.IsKnownWord(word) then return false end
    if Blacklist[word] then return false end
    if env.WordHelperDiscovery.Ignored[word] then return false end
...
-- Event-driven accepted-word recorder.
--
-- DISCOVERY RECORDER V2: synchronize character capture to Last Letter's actual
-- "Typed" sound instead of GUI ChildAdded events. The GUI rebuilds/recycles tiles
-- for scrolling and acceptance animations, which previously created false suffixes.
-- A Typed sound is the authoritative "one character was entered" pulse; the GUI is
-- sampled only to read that newest character. Once WPM signals acceptance, recording
-- freezes (with a tiny grace window for event ordering) so acceptance animations and
-- the next turn can never extend the learned word.
--
-- Safety policy: if a correction/deletion or other ambiguous mutation is observed
-- before acceptance, skip that submission rather than guessing.

env.WordHelperDiscovery.RecorderDisconnectAll = function()
    local connections = env.WordHelperDiscovery.RecorderConnections or {}
    for _, connection in ipairs(connections) do
        pcall(function() connection:Disconnect() end)
    end
    env.WordHelperDiscovery.RecorderConnections = {}
    env.WordHelperDiscovery.RecorderTracked = {}
    env.WordHelperDiscovery.RecorderContainer = nil
end

env.WordHelperDiscovery.RecorderAddConnection = function(connection)
    if not connection then return end
    env.WordHelperDiscovery.RecorderConnections = env.WordHelperDiscovery.RecorderConnections or {}
    table.insert(env.WordHelperDiscovery.RecorderConnections, connection)
end

env.WordHelperDiscovery.GetVisibleRecorderWord = function(frame)
    local container = frame and frame:FindFirstChild("CurrentWord")
    if not container then return "", false end
    ...
    for _, child in ipairs(container:GetChildren()) do
        ...
        if index and child:IsA("GuiObject") and letter and letter:IsA("TextLabel") then
            local value = tostring(letter.Text or ""):lower():gsub("[^a-z]", "")
            if #value > 0 then
                indexed[#indexed + 1] = {Index = index, Value = value:sub(1, 1)}
            end
        end
    end
    ...
    for _, entry in ipairs(indexed) do
        if expected and entry.Index ~= expected then hasGap = true end
        expected = entry.Index + 1
        pieces[#pieces + 1] = entry.Value
    end

    return table.concat(pieces), hasGap
end

env.WordHelperDiscovery.RecorderSeed = function(frame, now)
    local visible, hasGap = env.WordHelperDiscovery.GetVisibleRecorderWord(frame)
    if hasGap or #visible < 1 then
        env.WordHelperDiscovery.RecorderAmbiguous = true
        env.WordHelperDiscovery.RecorderReady = false
        return false
    end

    env.WordHelperDiscovery.RecorderBuffer = visible
    env.WordHelperDiscovery.RecorderBufferAt = now or tick()
    env.WordHelperDiscovery.RecorderReady = true
    return true
end

env.WordHelperDiscovery.RecorderNewestVisibleLetter = function(frame)
    local container = frame and frame:FindFirstChild("CurrentWord")
    ...
    for _, child in ipairs(container:GetChildren()) do
        local index = tonumber(child.Name)
        local letter = child:FindFirstChild("Letter")
        if index and index > bestIndex and letter and letter:IsA("TextLabel") then
            local value = tostring(letter.Text or ""):lower():gsub("[^a-z]", "")
            if #value > 0 then
                bestIndex = index
                best = value:sub(1, 1)
...
env.WordHelperDiscovery.RecorderHandleTypedSignal = function(source)
    local now = tick()

    -- Played and Playing can both fire for the same sound playback. Deduplicate them.
    local lastSignal = tonumber(env.WordHelperDiscovery.LastTypedSoundSignalAt or 0) or 0
    if (now - lastSignal) < 0.035 then return end
    env.WordHelperDiscovery.LastTypedSoundSignalAt = now

    if env.WordHelperDiscovery.RecorderReady ~= true
        or env.WordHelperDiscovery.RecorderAmbiguous == true then
        return
    end

    local pulseAt = tonumber(env.WordHelperDiscovery.AcceptancePulseAt or 0) or 0
    if pulseAt > 0 and (now - pulseAt) > 0.14 then
        -- Acceptance is already established. Never let animation/next-turn sounds extend it.
        return
    end

    local frame = env.WordHelperDiscovery.ActiveFrame
    if not frame or not frame.Parent then return end

    -- First try a clean full-prefix extension. This is ideal before truncation begins.
    local visible, hasGap = env.WordHelperDiscovery.GetVisibleRecorderWord(frame)
    local buffer = tostring(env.WordHelperDiscovery.RecorderBuffer or "")
    if not hasGap and #visible > #buffer and visible:sub(1, #buffer) == buffer then
        env.WordHelperDiscovery.RecorderBuffer = visible
        env.WordHelperDiscovery.RecorderBufferAt = now
    else
        -- Once the display scrolls/truncates, numeric GUI positions are no longer logical
        -- positions. The Typed pulse tells us exactly one new character was entered, so
        -- append only the rightmost currently-visible character.
        local newest = env.WordHelperDiscovery.RecorderNewestVisibleLetter(frame)
        if newest == "" then
            env.WordHelperDiscovery.RecorderAmbiguous = true
            env.WordHelperDiscovery.AcceptanceEventValid = false
            return
        end
        env.WordHelperDiscovery.RecorderBuffer = buffer .. newest
        env.WordHelperDiscovery.RecorderBufferAt = now
    end

    if pulseAt > 0 and (now - pulseAt) <= 0.14 then
        env.WordHelperDiscovery.AcceptanceEventBuffer = tostring(env.WordHelperDiscovery.RecorderBuffer or "")
        env.WordHelperDiscovery.AcceptanceEventBufferAt = now
        env.WordHelperDiscovery.AcceptanceEventValid =
            env.WordHelperDiscovery.RecorderAmbiguous ~= true
            and #env.WordHelperDiscovery.AcceptanceEventBuffer >= 2
    end
end

env.WordHelperDiscovery.AttachTypedSound = function(sound)
    if not (sound and sound:IsA("Sound")) then return false end
    if env.WordHelperDiscovery.TypedSound == sound and env.WordHelperDiscovery.TypedSoundConnected == true then
        return true
    end
    ...
    env.WordHelperDiscovery.TypedSound = sound

    local okPlayed, playedConnection = pcall(function()
        return sound.Played:Connect(function()
            env.WordHelperDiscovery.RecorderHandleTypedSignal("Played")
        end)
    end)
    if okPlayed and playedConnection then
        table.insert(env.WordHelperDiscovery.TypedSoundConnections, playedConnection)
    end

    local okPlaying, playingConnection = pcall(function()
        return sound:GetPropertyChangedSignal("Playing"):Connect(function()
            if sound.Playing then
                env.WordHelperDiscovery.RecorderHandleTypedSignal("Playing")
            end
        end)
    end)
    if okPlaying and playingConnection then
        table.insert(env.WordHelperDiscovery.TypedSoundConnections, playingConnection)
    end

    env.WordHelperDiscovery.TypedSoundConnected =
        #env.WordHelperDiscovery.TypedSoundConnections > 0
    return env.WordHelperDiscovery.TypedSoundConnected
end

env.WordHelperDiscovery.EnsureTypedSound = function()
    if env.WordHelperDiscovery.TypedSoundConnected == true
        and env.WordHelperDiscovery.TypedSound
        and env.WordHelperDiscovery.TypedSound.Parent then
        return true
    end

    local soundService = game:GetService("SoundService")
    local gameSounds = soundService and soundService:FindFirstChild("Game")
    local typed = gameSounds and gameSounds:FindFirstChild("Typed")
    if typed and env.WordHelperDiscovery.AttachTypedSound(typed) then
        return true
    end

    if not env.WordHelperDiscovery.TypedSoundDescendantConnection then
        env.WordHelperDiscovery.TypedSoundDescendantConnection = soundService.DescendantAdded:Connect(function(obj)
            if obj:IsA("Sound") and obj.Name == "Typed" then
                local parent = obj.Parent
                if parent and parent.Name == "Game" then
                    env.WordHelperDiscovery.AttachTypedSound(obj)
                end
            end
        end)
    end
...
env.WordHelperDiscovery.RecorderTrackChild = function(child)
    if not child or not child:IsA("GuiObject") then return end
    env.WordHelperDiscovery.RecorderTracked = env.WordHelperDiscovery.RecorderTracked or {}
    if env.WordHelperDiscovery.RecorderTracked[child] then return end
    env.WordHelperDiscovery.RecorderTracked[child] = true

    -- A Deleted rename before acceptance means the player corrected/backspaced or the
    -- UI performed an ambiguous mutation. We intentionally skip that submission rather
    -- than risk learning a misspelling. After acceptance, deletion is only animation.
    env.WordHelperDiscovery.RecorderAddConnection(
        child:GetPropertyChangedSignal("Name"):Connect(function()
            if child.Name == "Deleted" then
                local pulseAt = tonumber(env.WordHelperDiscovery.AcceptancePulseAt or 0) or 0
                if pulseAt <= 0 then
                    env.WordHelperDiscovery.RecorderAmbiguous = true
                    env.WordHelperDiscovery.AcceptanceEventValid = false
...
env.WordHelperDiscovery.RecorderBeginTurn = function(frame, typeText, now)
    env.WordHelperDiscovery.ActiveFrame = frame
    env.WordHelperDiscovery.RecorderBuffer = ""
    env.WordHelperDiscovery.RecorderBufferAt = 0
    env.WordHelperDiscovery.RecorderAmbiguous = false
    env.WordHelperDiscovery.RecorderReady = false
    env.WordHelperDiscovery.RecorderReadyAt = (now or tick()) + 0.06
    env.WordHelperDiscovery.RecorderTypeText = tostring(typeText or "")
    env.WordHelperDiscovery.LastTypedSoundSignalAt = 0

    env.WordHelperDiscovery.AcceptancePulseAt = 0
    env.WordHelperDiscovery.AcceptanceEventBuffer = ""
    env.WordHelperDiscovery.AcceptanceEventBufferAt = 0
    env.WordHelperDiscovery.AcceptanceEventValid = false
end

env.WordHelperDiscovery.RecorderAttach = function(frame, typeText, now)
    local container = frame and frame:FindFirstChild("CurrentWord")
    if not container then return false end

    env.WordHelperDiscovery.EnsureTypedSound()

    if env.WordHelperDiscovery.RecorderContainer == container then
        env.WordHelperDiscovery.ActiveFrame = frame
        return true
    end

    env.WordHelperDiscovery.RecorderDisconnectAll()
    env.WordHelperDiscovery.RecorderContainer = container
    env.WordHelperDiscovery.RecorderConnections = {}
    env.WordHelperDiscovery.RecorderTracked = {}
    env.WordHelperDiscovery.ActiveFrame = frame
    ...
    env.WordHelperDiscovery.RecorderAddConnection(
        container.ChildAdded:Connect(function(child)
            env.WordHelperDiscovery.RecorderTrackChild(child)
        end)
    )
    for _, child in ipairs(container:GetChildren()) do
        env.WordHelperDiscovery.RecorderTrackChild(child)
    ...
    env.WordHelperDiscovery.RecorderBeginTurn(frame, typeText, now)
    return true
end

env.WordHelperDiscovery.ObserveFrame = function(detectedWord, censored, typeText, now, frame)
    if not frame or censored then return end
    typeText = tostring(typeText or "")
    now = tonumber(now) or tick()

    if not env.WordHelperDiscovery.RecorderAttach(frame, typeText, now) then return end

    if env.WordHelperDiscovery.RecorderReady ~= true
        and now >= (tonumber(env.WordHelperDiscovery.RecorderReadyAt or 0) or 0) then
        env.WordHelperDiscovery.RecorderSeed(frame, now)
    end
    ...
    elseif typeText ~= env.WordHelperDiscovery.LastTypeText then
        ...
        local candidate = tostring(env.WordHelperDiscovery.AcceptanceEventBuffer or "")
        local valid = env.WordHelperDiscovery.AcceptanceEventValid == true
            and env.WordHelperDiscovery.RecorderAmbiguous ~= true

        if pulseAt > 0
            and (now - pulseAt) <= 1.25
            and wrongAt < pulseAt
            and valid
            and #candidate >= 2 then
            env.WordHelperDiscovery.ConsiderAccepted(
                candidate,
                "Typed-sound synchronized recorder + WPM + turn change"
            )
        end
    ...
    local wpm = frame:FindFirstChild("WPM")
    if not (wpm and wpm:IsA("TextLabel")) then return end

    local wpmText = tostring(wpm.Text or "")
    if env.WordHelperDiscovery.LastWPMText == "" then
        env.WordHelperDiscovery.LastWPMText = wpmText
        return
    end

    if wpmText ~= env.WordHelperDiscovery.LastWPMText then
        env.WordHelperDiscovery.LastWPMText = wpmText
        local amount = tonumber(wpmText:match("([%d%.]+)"))
        if amount and amount > 0
            and env.WordHelperDiscovery.RecorderReady == true
            and env.WordHelperDiscovery.RecorderAmbiguous ~= true then
            env.WordHelperDiscovery.AcceptancePulseAt = now
            env.WordHelperDiscovery.AcceptanceEventBuffer = tostring(env.WordHelperDiscovery.RecorderBuffer or "")
            env.WordHelperDiscovery.AcceptanceEventBufferAt = now
            env.WordHelperDiscovery.AcceptanceEventValid =
                #env.WordHelperDiscovery.AcceptanceEventBuffer >= 2
        end
    end
end
...
do
    ...
    local function SearchWords()
        ...
        for i, w in ipairs(results) do
            ...
            trapBtn.Size = UDim2.new(0, 80, 0, 18)
            trapBtn.Position = UDim2.new(1, -84, 0.5, -9)
    ...
    local function AddWordFromBrowser()
        ...
        local first = word:sub(1, 1)
        local existingBucket = Buckets and Buckets[first]
        if existingBucket then
            for _, existing in ipairs(existingBucket) do
                if existing == word then
    ...
    if #matches > 0 then
        ...
        elseif sortMode == "Godmode" then
            table.sort(matches, function(a, b)
                local sA = GetGodmodeScore(a)
                local sB = GetGodmodeScore(b)
                if sA == sB then
                    return #a < #b
                end
                return sA > sB
...
env.WordHelperBlacklistTracker.RejectionWindow = 1.05
...
env.WordHelperBlacklistTracker.IsKnownWord = function(word)
    ...
    -- Buckets are sorted, but a simple scan is robust even after custom additions.
...
env.WordHelperBlacklistTracker.BeginAttempt = function(word, requiredLetters)
    ...
    -- A wrong starting prefix is a typing/input mistake, not a dictionary rejection.
    if requiredLetters ~= ""
        and word:sub(1, #requiredLetters) ~= requiredLetters then
        return false
    end
...
env.WordHelperBlacklistTracker.HandleWrongSound = function(source)
    ...
    local stillMyTurn, nowRequired = GetTurnInfo(frame)
    if not stillMyTurn then return end
    nowRequired = tostring(nowRequired or ""):lower():gsub("[^a-z]", "")

    local candidate = select(1, GetCurrentGameWord(frame)) or ""
    candidate = tostring(candidate):lower():gsub("[^a-z]", "")

    local cached = tostring(env.WordHelperBlacklistTracker.LastPreSubmitWord or ""):lower():gsub("[^a-z]", "")
    local cachedAt = tonumber(env.WordHelperBlacklistTracker.LastPreSubmitAt or 0) or 0
    if #cached >= 2 and (tick() - cachedAt) <= 1.75 then
        if #candidate < 2 or #cached >= #candidate then candidate = cached end
    end

    if #candidate < 2 then
        if ShowToast then ShowToast("Wrong sound seen - no word captured", "warning") end
        return
    end
    if Blacklist[candidate] or UsedWords[candidate] then return end
    if not env.WordHelperBlacklistTracker.IsKnownWord(candidate) then
        if ShowToast then ShowToast("Wrong sound seen - unknown word: " .. candidate, "warning") end
        return
    end
    if nowRequired ~= "" and candidate:sub(1, #nowRequired) ~= nowRequired then
        if ShowToast then ShowToast("Wrong sound seen - prefix mismatch: " .. candidate, "warning") end
        return
    end

    local added = env.WordHelperBlacklistTracker.Add(candidate, "confirmed SoundService.Game.Wrong via " .. tostring(source or "sound"))
    if added then
        if ShowToast then ShowToast("Rejected word permanently blacklisted: " .. candidate, "warning") end
        if StatusText then
            StatusText.Text = "Blacklisted from Wrong sound: " .. candidate
            StatusText.TextColor3 = THEME.Warning
        end
    end
end
...
runConn = RunService.RenderStepped:Connect(function()
    local success, err = pcall(function()
        ...
        if isVisible then
            if detected ~= "" and not censored then
                if env.WordHelperUsedTracker.observedWord == "" or detected:sub(1, #env.WordHelperUsedTracker.observedWord) == env.WordHelperUsedTracker.observedWord then
                    if env.WordHelperUsedTracker.IsKnownDictionaryWord(detected) then
                        env.WordHelperUsedTracker.observedWord = detected
                        env.WordHelperUsedTracker.observedWordSince = now
                    end
                elseif detected ~= env.WordHelperUsedTracker.observedWord then
                    env.WordHelperUsedTracker.RecordObservedUsedWord(env.WordHelperUsedTracker.observedWord)
                    env.WordHelperUsedTracker.observedWord = env.WordHelperUsedTracker.IsKnownDictionaryWord(detected) and detected or ""
                    env.WordHelperUsedTracker.observedWordSince = now
                end
            elseif env.WordHelperUsedTracker.observedWord ~= "" and ((now - env.WordHelperUsedTracker.observedWordSince) > 0.08 or typeText ~= env.WordHelperUsedTracker.lastObservedTypeText) then
                env.WordHelperUsedTracker.RecordObservedUsedWord(env.WordHelperUsedTracker.observedWord)
                env.WordHelperUsedTracker.observedWord = ""
            end
        ...
        if seconds ~= nil or typeVisible or detected ~= "" or isMyTurn then
        ...
        elseif env.WordHelperUsedTracker.roundHadActivity then
            ...
            elseif (now - env.WordHelperUsedTracker.inactiveSince) >= 2.5 then
                table.clear(UsedWords)
                env.WordHelperUsedTracker.observedWord = ""
                env.WordHelperUsedTracker.observedWordSince = 0
                env.WordHelperUsedTracker.roundHadActivity = false
...
-- Temporary F7 accepted-word deep inspector.
-- Press F7 immediately before typing a known accepted word. Discovery commits are
-- disabled in this diagnostic build so bad candidates cannot pollute the JSON file.
env.WordHelperUIDebugger = env.WordHelperUIDebugger or {}
env.WordHelperUIDebugger.Active = false
env.WordHelperUIDebugger.FileName = "WordHelper_AcceptedWord_DeepDebug.txt"
env.WordHelperUIDebugger.Duration = 8.0
env.WordHelperUIDebugger.PollRate = 0.025

env.WordHelperUIDebugger.GetPath = function(obj)
    local ok, name = pcall(function() return obj:GetFullName() end)
    return ok and name or tostring(obj and obj.Name or "<unknown>")
end

env.WordHelperUIDebugger.GetAttributesString = function(obj)
    local ok, attrs = pcall(function() return obj:GetAttributes() end)
    if not ok or type(attrs) ~= "table" then return "" end
    local keys = {}
    for k in pairs(attrs) do table.insert(keys, tostring(k)) end
    table.sort(keys)
    local parts = {}
    for _, k in ipairs(keys) do
        ...
        if t == "string" or t == "number" or t == "boolean" then
            table.insert(parts, k .. "=" .. string.format("%q", tostring(v)))
        elseif t == "Instance" then
            table.insert(parts, k .. "=<Instance:" .. env.WordHelperUIDebugger.GetPath(v) .. ">")
        end
    end
    return table.concat(parts, ";")
end

env.WordHelperUIDebugger.ReadState = function(obj)
    local state = {
        ClassName = obj.ClassName,
        Path = env.WordHelperUIDebugger.GetPath(obj),
        Attributes = env.WordHelperUIDebugger.GetAttributesString(obj)
    }
    ...
    if obj:IsA("ValueBase") then
        ...
        if ok then
            if typeof(value) == "Instance" then
                state.Value = "<Instance:" .. env.WordHelperUIDebugger.GetPath(value) .. ">"
            else
                state.Value = tostring(value)
            end
        end
    end
    ...
    if obj:IsA("ScreenGui") then state.Enabled = obj.Enabled end
    if obj:IsA("ImageLabel") or obj:IsA("ImageButton") then state.Image = tostring(obj.Image or "") end
    if obj:IsA("Sound") then
        state.SoundId = tostring(obj.SoundId or "")
        state.Playing = obj.Playing
        state.TimePosition = obj.TimePosition
    end
    return state
end
...
env.WordHelperUIDebugger.StateToString = function(state)
    ...
    local parts = {"class=" .. tostring(state.ClassName), "path=" .. tostring(state.Path)}
    if state.Value ~= nil then table.insert(parts, "value=" .. string.format("%q", state.Value)) end
    if state.Text ~= nil then table.insert(parts, "text=" .. string.format("%q", state.Text)) end
    if state.Visible ~= nil then table.insert(parts, "visible=" .. tostring(state.Visible)) end
    if state.Enabled ~= nil then table.insert(parts, "enabled=" .. tostring(state.Enabled)) end
    if state.Image and state.Image ~= "" then table.insert(parts, "image=" .. string.format("%q", state.Image)) end
    if state.SoundId and state.SoundId ~= "" then table.insert(parts, "soundId=" .. string.format("%q", state.SoundId)) end
    if state.Playing ~= nil then table.insert(parts, "playing=" .. tostring(state.Playing)) end
    if state.Attributes and state.Attributes ~= "" then table.insert(parts, "attrs={" .. state.Attributes .. "}") end
    return table.concat(parts, " | ")
end

env.WordHelperUIDebugger.StatesDiffer = function(a,b)
    if not a or not b then return true end
    return a.ClassName ~= b.ClassName or a.Path ~= b.Path or a.Value ~= b.Value
        or a.Text ~= b.Text or a.Visible ~= b.Visible or a.Enabled ~= b.Enabled
        or a.Image ~= b.Image or a.SoundId ~= b.SoundId or a.Playing ~= b.Playing
        or a.Attributes ~= b.Attributes
end

env.WordHelperUIDebugger.ShouldTrack = function(obj, roots)
    if not obj then return false end
    if obj:IsA("ValueBase") or obj:IsA("Sound") then return true end
    if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then return true end
    local attrs = env.WordHelperUIDebugger.GetAttributesString(obj)
    if attrs ~= "" then return true end
    -- Keep other GUI structure only inside InGame so added/removed word-related objects appear.
    if roots and roots.InGame and obj:IsDescendantOf(roots.InGame) and (obj:IsA("GuiObject") or obj:IsA("UIStroke")) then
        return true
    end
    return false
end

env.WordHelperUIDebugger.GetRoots = function()
    local p = Players.LocalPlayer
    local pg = p and p:FindFirstChild("PlayerGui")
    local roots = {
        LocalPlayer = p,
        PlayerGui = pg,
        InGame = pg and pg:FindFirstChild("InGame"),
...
env.WordHelperUIDebugger.Snapshot = function()
    local roots = env.WordHelperUIDebugger.GetRoots()
    local snap = {}
    local visited = {}
    local function scan(root)
        if not root or visited[root] then return end
        visited[root] = true
        if env.WordHelperUIDebugger.ShouldTrack(root, roots) then snap[root] = env.WordHelperUIDebugger.ReadState(root) end
        for _, obj in ipairs(root:GetDescendants()) do
            if env.WordHelperUIDebugger.ShouldTrack(obj, roots) then
                snap[obj] = env.WordHelperUIDebugger.ReadState(obj)
            end
        end
    end
    scan(roots.PlayerGui)
    scan(roots.LocalPlayer)
    scan(roots.ReplicatedStorage)
    scan(roots.SoundService)
    return snap, roots
end

env.WordHelperUIDebugger.Start = function()
    if env.WordHelperUIDebugger.Active then
        if ShowToast then ShowToast("F7 accepted-word capture already running", "warning") end
        return
    end
    if not writefile then
        if ShowToast then ShowToast("Executor does not support writefile", "error") end
        return
    end

    local before, roots = env.WordHelperUIDebugger.Snapshot()
    if not roots.InGame then
        if ShowToast then ShowToast("InGame UI not found - enter a match first", "warning") end
        return
    end

    env.WordHelperUIDebugger.Active = true
    if ShowToast then ShowToast("F7 ACCEPTED capture started - type a valid word now", "success") end
    if StatusText then
        StatusText.Text = "F7 accepted-word deep capture running..."
        StatusText.TextColor3 = THEME.Warning
    end
    ...
    task.spawn(function()
        ...
        local function soundString(sound)
            local okPath, path = pcall(function() return sound:GetFullName() end)
            if not okPath then path = tostring(sound.Name) end
        ...
        local function logSound(kind, sound)
            table.insert(soundEvents, string.format("[+%.3fs] %s | %s", tick()-started, kind, soundString(sound)))
        end
        local function watchSound(sound)
            ...
            local ok1,c1=pcall(function() return sound.Played:Connect(function() logSound("PLAYED",sound) end) end)
            if ok1 and c1 then table.insert(soundConnections,c1) end
            local ok2,c2=pcall(function() return sound:GetPropertyChangedSignal("Playing"):Connect(function()
                if sound.Playing then logSound("PLAYING_TRUE",sound) end
            end) end)
            if ok2 and c2 then table.insert(soundConnections,c2) end
        end
        for _, obj in ipairs(game:GetDescendants()) do if obj:IsA("Sound") then watchSound(obj) end end
        local dc = game.DescendantAdded:Connect(function(obj)
            if obj:IsA("Sound") then watchSound(obj); logSound("SOUND_ADDED",obj) end
        end)
        table.insert(soundConnections,dc)

        table.insert(lines,"WordHelper Accepted Word Deep Inspector")
        table.insert(lines,"Discovery commits disabled: true")
        ...
        table.insert(lines,"Tracked initial objects: "..tostring((function() local n=0 for _ in pairs(before) do n=n+1 end return n end)()))
        table.insert(lines,"")
        table.insert(lines,"=== INITIAL TEXT / VALUES / ATTRIBUTES ===")
        for _,state in pairs(before) do
            if state.Text ~= nil or state.Value ~= nil or (state.Attributes and state.Attributes ~= "") then
        ...
        while not unloaded and tick()-started < env.WordHelperUIDebugger.Duration do
            ...
            local current = env.WordHelperUIDebugger.Snapshot()
            for obj,oldState in pairs(previous) do
                local newState=current[obj]
                if not newState then
                    eventCount=eventCount+1
                    table.insert(lines,string.format("[+%.3fs] REMOVED | %s",tick()-started,env.WordHelperUIDebugger.StateToString(oldState)))
                elseif env.WordHelperUIDebugger.StatesDiffer(oldState,newState) then
                    eventCount=eventCount+1
                    table.insert(lines,string.format("[+%.3fs] CHANGED | BEFORE: %s",tick()-started,env.WordHelperUIDebugger.StateToString(oldState)))
            ...
            for obj,newState in pairs(current) do
                if not previous[obj] then
                    eventCount=eventCount+1
                    table.insert(lines,string.format("[+%.3fs] ADDED | %s",tick()-started,env.WordHelperUIDebugger.StateToString(newState)))
                end
            end
            previous=current
        end

        for _,c in ipairs(soundConnections) do pcall(function() c:Disconnect() end) end
        table.insert(lines,"")
        table.insert(lines,"=== SOUND EVENTS ===")
        for _,s in ipairs(soundEvents) do table.insert(lines,s) end
        if #soundEvents==0 then table.insert(lines,"<none>") end
        table.insert(lines,"")
        table.insert(lines,"=== FINAL TEXT / VALUES / ATTRIBUTES ===")
        ...
        table.insert(lines,"")
        table.insert(lines,"Total state changes: "..tostring(eventCount))
        table.insert(lines,"Total sound events: "..tostring(#soundEvents))

        local ok,err=pcall(function() writefile(env.WordHelperUIDebugger.FileName,table.concat(lines,"\n")) end)
        env.WordHelperUIDebugger.Active=false
        if ok then
            if ShowToast then ShowToast("Accepted-word capture saved: "..env.WordHelperUIDebugger.FileName,"success") end
            if StatusText then StatusText.Text="F7 accepted-word capture saved"; StatusText.TextColor3=THEME.Success end
        else
            if ShowToast then ShowToast("Accepted capture save failed","error") end
            if StatusText then StatusText.Text="F7 capture failed: "..tostring(err); StatusText.TextColor3=THEME.Warning end
        end
    end)
end
