-- V20.8.22: cached F9 spam scan + shortest representatives + diversified Pro fallback
        ...
        if needSpamScan then
            for ci, cat in ipairs(categories) do
                ...
                if cat.Kind == "TRAP" then
                    matchesCategory = GodmodeIsTrapWord(w)
                elseif cat.Kind == "X" then
                    matchesCategory = w:sub(-1) == "x"
                else
                    local ending = cat.Value
                    matchesCategory = #w >= #ending and w:sub(-#ending) == ending
                end

                if matchesCategory then
                    local current = categoryBest[ci]
                    if current == nil then
                        categoryBest[ci] = w
                    elseif cat.Kind == "ENDING" then
                        -- V20.8.21: literal spam is a typing-speed tool.
                        -- Pick the SHORTEST available word for this ending, not the
                        -- earliest alphabetically / strongest Pro candidate.
                        -- Equal-length words use the normal Pro comparator only as
                        -- a deterministic strategic tie-break.
                        if #w < #current or (#w == #current and better(w, current)) then
                            categoryBest[ci] = w
                        end
                    elseif better(w, current) then
                        -- TRAP / X keep their existing Pro strategic selection.
                        categoryBest[ci] = w
                    end
                    -- A word represents its highest Godmode category only.
                    break
                end
            end
        end
