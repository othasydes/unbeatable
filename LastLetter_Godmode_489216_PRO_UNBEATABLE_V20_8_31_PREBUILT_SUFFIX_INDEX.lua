-- V20.8.31: prebuilt Pro suffix metadata + no full suffix reconstruction on new-prefix queries
...
env.WordHelperUnbeatable.SelectTopProCandidates = function(candidateList)
    ...
    -- V20.8.31 NORMAL PRO INDEXED FULL-COVERAGE DISCOVERY.
    ...
    local discoveryKey = table.concat({
        "v31", "s" .. tostring(stage), tostring(#originalCandidateList),
        firstCandidate, lastCandidate,
        tostring(env.WordHelperUsedTracker and env.WordHelperUsedTracker.LastRecordedWord or "")
    }, "|")

    local cachedDiscovery = env.WordHelperUnbeatable.ProDiscoveryCache[discoveryKey]
    local shortlist, spicyStageTwo, regularStageTwo

    if cachedDiscovery then
        shortlist = {}
        spicyStageTwo = {}
        regularStageTwo = {}
        for i, w in ipairs(cachedDiscovery.Shortlist or {}) do shortlist[i] = w end
        for i, w in ipairs(cachedDiscovery.Spicy or {}) do spicyStageTwo[i] = w end
        for i, w in ipairs(cachedDiscovery.Regular or {}) do regularStageTwo[i] = w end
    else
        shortlist = {}
        spicyStageTwo = {}
        regularStageTwo = {}
        local shortlistSeen = {}
        local spicySeen, regularSeen = {}, {}
        local returnBest = {}
        local xBest = nil

        local function addShort(word)
            if word and not shortlistSeen[word] then
                shortlistSeen[word] = true
                shortlist[#shortlist + 1] = word
            end
        end
        local function easierTrigger(a, b)
            return b == nil or #a < #b or (#a == #b and a < b)
        end

        -- Full candidate coverage remains, but suffix construction/eligibility is
        -- now indexed per word and never recomputed on later prefix queries.
        for _, w in ipairs(originalCandidateList) do
            local meta = env.WordHelperUnbeatable.GetProWordSuffixMeta(w)
            if meta[1] and meta[1].Prefix == "x" and easierTrigger(w, xBest) then
                xBest = w
            end

            local chosenPrefix, chosenNonSelf, chosenSelf = nil, 999999, false
            for suffixLen = math.min(stage, #w), 1, -1 do
                local sm = meta[suffixLen]
                if sm and sm.Eligible then
                    local prefix = sm.Prefix
                    local baseCount, baseSelf =
                        env.WordHelperUnbeatable.GetProBaseReplyInfo(prefix)
                    local candidateWasCounted =
                        w:sub(1, #prefix) == prefix
                        and not env.WordHelperUnbeatable.IsProWordUnavailable(w)
                    local available = baseCount - (candidateWasCounted and 1 or 0)
                    local availableSelf = baseSelf and w ~= prefix
                    local otherReplies = available - (availableSelf and 1 or 0)
                    if otherReplies >= 1 then
                        chosenPrefix, chosenNonSelf, chosenSelf =
                            prefix, otherReplies, availableSelf
                        break
                    end
                end
            end
            ...
            if chosenPrefix then
                ...
                if old == nil or easierTrigger(w, old.Word) then
                    returnBest[chosenPrefix] = {
                        Word=w, Prefix=chosenPrefix, NonSelf=chosenNonSelf,
                        Self=chosenSelf,
                        Total=chosenNonSelf + (chosenSelf and 1 or 0)
                    }
                end
            end
        end
        ...
        if stage == 2 then
            ...
            for _,w in ipairs(spicyStageTwo) do addShort(w) end
            for _,w in ipairs(regularStageTwo) do addShort(w) end
        end

        addShort(xBest)
        for _,row in ipairs(discovered) do
            if row.NonSelf <= 2 then addShort(row.Word) end
        ...
        env.WordHelperUnbeatable.ProDiscoveryCache[discoveryKey] = {
            Shortlist=shortlist, Spicy=spicyStageTwo, Regular=regularStageTwo
        }
        table.insert(env.WordHelperUnbeatable.ProDiscoveryCacheOrder, discoveryKey)
        while #env.WordHelperUnbeatable.ProDiscoveryCacheOrder > 24 do
            local oldKey=table.remove(env.WordHelperUnbeatable.ProDiscoveryCacheOrder,1)
            env.WordHelperUnbeatable.ProDiscoveryCache[oldKey]=nil
        end
    end
