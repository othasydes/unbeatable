...
if env.WordHelperBlacklistTracker.WrongSound
    ...
    env.WordHelperBlacklistTracker.WrongSoundConnection =
        env.WordHelperBlacklistTracker.WrongSound.Played:Connect(function()
            ...
            local word = env.WordHelperBlacklistTracker.PendingWord
            local submittedAt = env.WordHelperBlacklistTracker.PendingAt
            if word == "" or submittedAt == 0 then return end

            local age = tick() - submittedAt
            if age < 0 or age > env.WordHelperBlacklistTracker.RejectionWindow then
                env.WordHelperBlacklistTracker.ClearPending()
                return
            end

            -- If the accepted-word observer has already seen it, this Wrong sound
            -- belongs to some other event/player and must not teach the blacklist.
            if UsedWords[word] then
                env.WordHelperBlacklistTracker.ClearPending()
                return
            end
            ...
            local expectedRequired =
                tostring(env.WordHelperBlacklistTracker.PendingRequired or "")
                    :lower():gsub("[^a-z]", "")

            -- Accepted submissions rotate the turn. A rejection keeps our turn.
            ...
            if expectedRequired ~= ""
                and nowRequired ~= ""
                and nowRequired ~= expectedRequired then
                env.WordHelperBlacklistTracker.ClearPending()
                return
            end
...
env.WordHelperBlacklistTracker.KeyConnection =
    UserInputService.InputBegan:Connect(function(input, processed)
        ...
        if #attemptedWord < 2 then return end
