-- V20.8.9: sticky long-word recommendation lock + SERVER-ONLY AnswerResults UsedWords tracking
...
env.WordHelperLongWord.Resolve = function(visibleText, requiredLetter)
    ...
    -- The visible letters no longer belong to the locked word: user changed course.
    state.Reset()
    return visible
end
...
UpdateList = function(detectedText, requiredLetter)
    ...
    if not manualSearch
        ...
        if lockedWord:sub(1, #searchPrefix) == searchPrefix
    ...
    if not manualSearch
        and env.WordHelperLongWord
        and env.WordHelperLongWord.Active
        and env.WordHelperLongWord.Word ~= ""
        and env.WordHelperLongWord.Word:sub(1, #searchPrefix) == searchPrefix then
