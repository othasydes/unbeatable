-- V20.8.15: punctuation-safe (' / -) server UsedWords + Pro canonical fix + backspace-safe sliding reconstruction
...
env.WordHelperLongWord.ReconstructSliding = function(previousText, rawText)
    ...
    local overflow = raw:find("%.%.%.") ~= nil
    local visible = raw:gsub("[^a-z]", "")

    -- Once enough letters are deleted, the ellipsis disappears and the UI again
    -- contains the authoritative complete prefix. Rebase immediately.
    if not overflow then
        state.SlidingFull = visible
        state.SlidingWindow = visible
        return visible
    end

    local full = tostring(state.SlidingFull or previous):lower():gsub("[^a-z]", "")
    local oldWindow = tostring(state.SlidingWindow or ""):lower():gsub("[^a-z]", "")
    if full == "" then full = previous end
    if visible == "" then return full end

    -- If the new window already occurs inside our reconstructed prefix, this is
    -- normally a backspace. Rebuild to the END of that window rather than appending.
    ...
    -- Normal forward typing: append only what moved beyond the previous suffix.
    ...
    -- Mistype/backspace recovery. Preserve the hidden left side while replacing the
    -- visible tail. A typo frame therefore cannot destroy reconstruction history.
    if oldWindow ~= "" then
        local oldPos = full:find(oldWindow, 1, true)
        if oldPos then
            local common, limit = 0, math.min(#oldWindow, #visible)
            for i=1,limit do
                if oldWindow:sub(i,i) ~= visible:sub(i,i) then break end
                common=i
            end
    ...
    -- Unknown transient/mistyped frame: retain the reconstructed prefix so that
    -- deleting the typo can recover on the next recognizable sliding window.
    state.SlidingWindow = visible
    return full
end

env.WordHelperLongWord.Resolve = function(visibleText, requiredLetter)
    -- V20.8.12: no hard lock. Always return the live reconstructed prefix.
    return tostring(visibleText or ""):lower():gsub("[^a-z]", "")
end
