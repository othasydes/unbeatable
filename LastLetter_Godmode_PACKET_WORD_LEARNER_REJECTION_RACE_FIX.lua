...
env.WordHelperBlacklistTracker.HandleWrongSound = function(source)
    ...
    -- Prefer the word explicitly armed by our Enter handler. BeginAttempt only
    -- arms words that were NOT already used before submission, so this safely
    -- avoids the race where the used-word observer marks a rejected word as
    -- "used" a fraction of a second before the Wrong sound arrives.
    local pendingWord = tostring(env.WordHelperBlacklistTracker.PendingWord or ""):lower():gsub("[^a-z]", "")
    local pendingAt = tonumber(env.WordHelperBlacklistTracker.PendingAt or 0) or 0
    local hasFreshPending = #pendingWord >= 2
        and (tick() - pendingAt) <= (env.WordHelperBlacklistTracker.RejectionWindow or 1.05)

    if hasFreshPending then
    ...
    -- If this exact attempt was armed before submission, ignore a UsedWords flag
    -- that may have been set by the observer after the word disappeared.
    if UsedWords[candidate] and not (hasFreshPending and candidate == pendingWord) then
        return
    end
    ...
    if hasFreshPending and candidate == pendingWord then
        env.WordHelperBlacklistTracker.ClearPending()
    end

    if added then
        -- Add() already shows the blacklist toast and refreshes the manager.
        if StatusText then
