...
-- TEMPORARY discovery diagnostics.
-- Records the exact runtime membership state whenever packet discovery reviews a candidate.
env.WordHelperDiscovery.DiagnosticFileName = "WordHelper_DiscoveryDiagnostics.txt"

env.WordHelperDiscovery.GetDiagnosticState = function(word)
    word = tostring(word or ""):lower():gsub("[^a-z]", "")

    local inKnownSet = WordHelperKnownWords and WordHelperKnownWords[word] == true or false
    local inWords = false
    ...
    if Config and Config.CustomWords then
        for _, existing in ipairs(Config.CustomWords) do
            local clean = tostring(existing or ""):lower():gsub("[^a-z]", "")
            if clean == word then
                inCustom = true
                break
            end
    ...
    return {
        ...
        bucket = inBucket,
        custom = inCustom,
        wordCount = Words and #Words or -1,
        bucketSize = bucketSize,
        blacklist = Blacklist and Blacklist[word] == true or false,
        ignored = env.WordHelperDiscovery.Ignored and env.WordHelperDiscovery.Ignored[word] == true or false,
        pendingBefore = env.WordHelperDiscovery.Pending and tonumber(env.WordHelperDiscovery.Pending[word]) or 0
    }
end

env.WordHelperDiscovery.LogDiagnostic = function(word, source, stage)
    local state = env.WordHelperDiscovery.GetDiagnosticState(word)
    local line = string.format(
        "[%s] stage=%s source=%s word=%s knownSet=%s words=%s bucket=%s custom=%s blacklist=%s ignored=%s pendingBefore=%s wordCount=%s bucketSize=%s",
        os.date("%Y-%m-%d %H:%M:%S"),
        tostring(stage or "?"),
        tostring(source or "?"),
        tostring(state.word),
        tostring(state.knownSet),
        tostring(state.words),
        tostring(state.bucket),
        tostring(state.custom),
        tostring(state.blacklist),
        tostring(state.ignored),
        tostring(state.pendingBefore),
        tostring(state.wordCount),
        tostring(state.bucketSize)
    )

    pcall(function()
        if appendfile then
            appendfile(env.WordHelperDiscovery.DiagnosticFileName, line .. "\n")
        elseif writefile then
            local old = ""
            if isfile and readfile and isfile(env.WordHelperDiscovery.DiagnosticFileName) then
                old = readfile(env.WordHelperDiscovery.DiagnosticFileName) or ""
            end
            writefile(env.WordHelperDiscovery.DiagnosticFileName, old .. line .. "\n")
        end
    end)

    pcall(function() print("[WordHelper Discovery Diagnostic] " .. line) end)
    return state
end
...
env.WordHelperDiscovery.ConsiderAccepted = function(word, source)
    ...
    env.WordHelperDiscovery.LogDiagnostic(word, source, "candidate")

    if env.WordHelperDiscovery.IsKnownWord(word) then
        env.WordHelperDiscovery.LogDiagnostic(word, source, "rejected_known")
        return false
    end
    if Blacklist[word] then
        env.WordHelperDiscovery.LogDiagnostic(word, source, "rejected_blacklist")
        return false
    end
    if env.WordHelperDiscovery.Ignored[word] then
        env.WordHelperDiscovery.LogDiagnostic(word, source, "rejected_ignored")
        return false
    end
    ...
    env.WordHelperDiscovery.LogDiagnostic(word, source, "accepted_unknown")
    env.WordHelperDiscovery.Save()
    env.WordHelperDiscovery.RefreshUI()
