-- V20.8.6: server-confirmed AnswerResults UsedWords tracking; UI/event capture retained as fallback
...
-- V20.8.5 EVENT-DRIVEN USED-WORD CAPTURE
-- RenderStepped can still miss an answer that exists for less than one rendered frame.
-- Listen directly to CurrentWord tile mutations and preserve the latest COMPLETE
-- dictionary word belonging to the current prompt until the turn transition confirms it.
env.WordHelperUsedTracker.EventCandidate = ""
env.WordHelperUsedTracker.EventCandidateAt = 0
env.WordHelperUsedTracker.EventCurrentWord = nil
env.WordHelperUsedTracker.EventConnections = {}

env.WordHelperUsedTracker.DisconnectEventConnections = function()
    for _, conn in ipairs(env.WordHelperUsedTracker.EventConnections or {}) do
...
env.WordHelperUsedTracker.CaptureMutation = function(frame)
    if unloaded or not frame then return end
    local word, censored = GetCurrentGameWord(frame)
    word = tostring(word or ""):lower():gsub("[%s%c]+", "")
    if censored or #word < 2 then return end
    if not env.WordHelperUsedTracker.IsKnownDictionaryWord(word) then return end

    -- Only retain a candidate that genuinely extends the currently guarded prompt.
    -- This prevents the next player's fresh returned prefix from replacing the answer
    -- during the tiny answer -> prompt -> Type transition race.
    local prompt = tostring(env.WordHelperUsedTracker.ActivePrompt or "")
        :lower():gsub("[%s%c]+", "")
    if prompt ~= "" then
        if word == prompt or word:sub(1, #prompt) ~= prompt then return end
    end

    env.WordHelperUsedTracker.EventCandidate = word
    env.WordHelperUsedTracker.EventCandidateAt = tick()
end

env.WordHelperUsedTracker.AttachEventCapture = function(frame)
    local currentWord = frame and frame:FindFirstChild("CurrentWord")
    if not currentWord or currentWord == env.WordHelperUsedTracker.EventCurrentWord then return end

    env.WordHelperUsedTracker.DisconnectEventConnections()
    env.WordHelperUsedTracker.EventCurrentWord = currentWord

    local function hookLetter(obj)
        if not obj:IsA("TextLabel") or obj.Name ~= "Letter" then return end
        table.insert(env.WordHelperUsedTracker.EventConnections,
            obj:GetPropertyChangedSignal("Text"):Connect(function()
                env.WordHelperUsedTracker.CaptureMutation(frame)
            end))
        table.insert(env.WordHelperUsedTracker.EventConnections,
    ...
    for _, obj in ipairs(currentWord:GetDescendants()) do hookLetter(obj) end
    table.insert(env.WordHelperUsedTracker.EventConnections,
        currentWord.DescendantAdded:Connect(function(obj)
            hookLetter(obj)
            env.WordHelperUsedTracker.CaptureMutation(frame)
        end))
    table.insert(env.WordHelperUsedTracker.EventConnections,
        currentWord.DescendantRemoving:Connect(function()
            env.WordHelperUsedTracker.CaptureMutation(frame)
        end))
    table.insert(env.WordHelperUsedTracker.EventConnections,
        currentWord.ChildAdded:Connect(function() env.WordHelperUsedTracker.CaptureMutation(frame) end))
    table.insert(env.WordHelperUsedTracker.EventConnections,
        currentWord.ChildRemoved:Connect(function() env.WordHelperUsedTracker.CaptureMutation(frame) end))
end
...
env.WordHelperUsedTracker.RecordObservedUsedWord = function(word, serverConfirmed)
    ...
    local activePrompt = tostring(env.WordHelperUsedTracker.ActivePrompt or "")
        :lower():gsub("[%s%c]+", "")
    if not serverConfirmed and activePrompt ~= "" and word == activePrompt then
        return
    end
    ...
    env.WordHelperUsedTracker.LastRecordedSource =
        serverConfirmed and "Server AnswerResults" or "UI fallback"
...
runConn = RunService.RenderStepped:Connect(function()
    local success, err = pcall(function()
        ...
        env.WordHelperUsedTracker.AttachEventCapture(frame)
        ...
        if isVisible then
            ...
            elseif env.WordHelperUsedTracker.observedWord ~= "" then
                ...
                -- V20.8.4 ULTRA-FAST ACCEPTANCE FIX:
                -- A Type/turn transition is already our positive acceptance signal.
                -- Do not require the completed word to remain visible for 80 ms:
                -- fast opponents can submit and advance in less than that window.
                if typeText ~= env.WordHelperUsedTracker.lastObservedTypeText then
                    env.WordHelperUsedTracker.RecordObservedUsedWord(env.WordHelperUsedTracker.observedWord)
                    env.WordHelperUsedTracker.observedWord = ""
                end
            end

            -- V20.8.5: on a confirmed turn/Type transition, prefer the event-captured
            -- complete answer. It survives even when the entire accepted word appeared
            -- and disappeared between two RenderStepped samples.
            if typeText ~= env.WordHelperUsedTracker.lastObservedTypeText then
                local eventWord = tostring(env.WordHelperUsedTracker.EventCandidate or "")
                if eventWord ~= "" then
                    env.WordHelperUsedTracker.RecordObservedUsedWord(eventWord)
                elseif env.WordHelperUsedTracker.observedWord ~= "" then
                    env.WordHelperUsedTracker.RecordObservedUsedWord(env.WordHelperUsedTracker.observedWord)
                end
                env.WordHelperUsedTracker.EventCandidate = ""
