-- V20.8.20: F9 diversified Pro choices: one best option per priority category, then Pro lowest-response fallback
...
env.WordHelperUnbeatable.SelectTopProCandidates = function(candidateList)
        ...
        if env.WordHelperUnbeatable.ProSpamMode then
            for ci, cat in ipairs(categories) do
                ...
                if matchesCategory then
                    ...
                    if current == nil or better(w, current) then
                        categoryBest[ci] = w
                    end
    ...
    --   category #1 best -> category #2 best -> ... -> normal Pro fallback.
