...
-- V20.9.10 OFFICIAL: shortest spam triggers guaranteed in BOTH Casual Godmode and Pro Spam
-- V20.9.9 OFFICIAL: Pro Spam always chooses shortest available trigger for each spam ending
-- V20.9.8 OFFICIAL: XW/RW confirmed Pro traps with absolute Stage-2 trap priority
-- V20.9.7 OFFICIAL: Pro Spicy Stage 4 mixes true 3-letter + 4-letter beast returns
-- V20.9.6 OFFICIAL: register-safe Casual validation (fixes 200-local compile ceiling)
-- V20.9.5 OFFICIAL: corrected Casual GitHub source + 478,372-word load validation
-- V20.9.4 OFFICIAL: Casual dictionary expanded to 478,372 confirmed alphabetic words
...
local fileName = "LastLetterLibrary_CASUAL_478372_CONFIRMED.txt"

-- V17 external dictionary source.
-- Upload LastLetterLibrary_CASUAL_478372_CONFIRMED.txt to this path in your existing GitHub repo.
-- WordHelper uses the cached local file whenever available, so normal launches do
-- not repeatedly download 476k words.
local dictionaryUrl =
    "https://raw.githubusercontent.com/othasydes/unbeatable/refs/heads/main/LastLetterLibrary_CASUAL_478372_CONFIRMED.txt"
...
UpdateStatus("Loading 478,372 Casual + Pro delta...", THEME.Warning)
...
-- V20.9.6: same validation, but deliberately NO new main-chunk local.
-- This file already sits at Luau's 200-local/register ceiling.
if #Words < 478372 then
    UpdateStatus(
        "CASUAL DICTIONARY ERROR: " .. tostring(#Words) .. " / 478372 words loaded",
        THEME.Warning
    )
    error(
        "WordHelper V20.9.6 Casual dictionary incomplete: "
        .. tostring(#Words) .. "/478372"
    )
end
UpdateStatus(
    "CASUAL VERIFIED: " .. tostring(#Words) .. " / 478372 words loaded!",
    THEME.Success
)
...
Title.Text = "<font color=\"rgb(45,255,112)\">qoox\'s</font> wordhelper <font color=\"rgb(105,166,121)\">// V20.9.10</font>"
Title.RichText = true
...
env.WordHelperUnbeatable.ProGodmodeTwoLetterTraps = {
    ...
    mv=true, vp=true, rz=true, pm=true, xw=true, rw=true
}
...
-- V20.9.8: XW/RW outrank every other confirmed Stage-2 trap.
env.WordHelperUnbeatable.ProGodmodeTopTwoLetterTraps = {
    xw=true, rw=true
}
...
env.WordHelperUnbeatable.SelectTopProCandidates = function(candidateList)
    ...
    if env.WordHelperUnbeatable.ProSpamMode then
        ...
        local result, seen = {}, {}
        local categoryBest = {}
        local topTrapPool, priorityTrapPool, normalTrapPool = {}, {}, {}
        local topTrapSeen, priorityTrapSeen, normalTrapSeen = {}, {}, {}
        local ordinaryPool = {}
        ...
        for _, w in ipairs(candidateList) do
            if stage == 2 and #w >= 2 then
                ...
                if env.WordHelperUnbeatable.ProGodmodeTwoLetterTraps[ending]
                    or env.WordHelperUnbeatable.ProGodmodePriorityTwoLetterTraps[ending]
                    or env.WordHelperUnbeatable.ProGodmodeTopTwoLetterTraps[ending] then
                    local target = env.WordHelperUnbeatable.ProGodmodeTopTwoLetterTraps[ending]
                        and topTrapPool
                        or (env.WordHelperUnbeatable.ProGodmodePriorityTwoLetterTraps[ending]
                            and priorityTrapPool or normalTrapPool)
                    local targetSeen = env.WordHelperUnbeatable.ProGodmodeTopTwoLetterTraps[ending]
                        and topTrapSeen
                        or (env.WordHelperUnbeatable.ProGodmodePriorityTwoLetterTraps[ending]
        ...
        if stage == 2 then
            local trapCount = 0
            for _, w in ipairs(topTrapPool) do
        ...
        for _, w in ipairs(topTrapPool) do consider(w) end
    ...
    if reverse then
        ...
        -- V20.9.7 F8 pool:
        -- S3 keeps the established exact-3 2-10 strategy.
        -- S4 deliberately mixes ACTUAL returned 3- and 4-letter beasts.
        -- A 3-letter row can only exist here when the stage-4 suffix did NOT have
        -- a valid continuation, because chosenPrefix above always takes the
        -- longest stage-legal suffix first. This prevents "ting" accidentally
        -- being treated as an "ing" trigger. S4 also admits 1-answer beasts such
        -- as the user's "eez" example, while still capping the pool at 10.
        if stage==3 or stage==4 then
            local obscureRows={}
            for _,row in ipairs(discovered) do
                local spicyLengthOK = (stage==3 and #row.Prefix==3)
                    or (stage==4 and (#row.Prefix==3 or #row.Prefix==4))
                local spicyMinimum = stage==4 and 1 or 2
                if spicyLengthOK and row.Total>=spicyMinimum and row.Total<=10 then
        ...
        if stage==2 then
            for _,row in ipairs(discovered) do
                local p=row.Prefix
                if #p==2 and env.WordHelperUnbeatable.ProGodmodeTopTwoLetterTraps[p]
                    and not spicySeen[p] then
                    spicySeen[p]=true; table.insert(spicyStageTwo, 1, row.Word)
                elseif #p==2 and env.WordHelperUnbeatable.ProGodmodePriorityTwoLetterTraps[p]
                    and not spicySeen[p] then
    ...
    local function better(a, b)
        ...
        if env.WordHelperUnbeatable.ProSpicyRandom then
            ...
            if spicyStage == 2 then
                ...
                local aTop = #pa==2 and env.WordHelperUnbeatable.ProGodmodeTopTwoLetterTraps[pa] == true
                local bTop = #pb==2 and env.WordHelperUnbeatable.ProGodmodeTopTwoLetterTraps[pb] == true
            ...
            elseif spicyStage == 3 or spicyStage == 4 then
                ...
                -- V20.9.7: at S4, actual 3- and 4-letter returns are peers.
                -- GetProCandidateInfo already resolves the longest valid suffix,
                -- so a 3-letter return here is guaranteed not to be swallowed by
                -- a valid 4-letter continuation.
                local obscureA = (
                    (spicyStage==3 and #prefixA==3 and totalA>=2 and totalA<=10)
                    or (spicyStage==4 and (#prefixA==3 or #prefixA==4) and totalA>=1 and totalA<=10)
                )
                local obscureB = (
                    (spicyStage==3 and #prefixB==3 and totalB>=2 and totalB<=10)
                    or (spicyStage==4 and (#prefixB==3 or #prefixB==4) and totalB>=1 and totalB<=10)
                )
    ...
    for _, w in ipairs(candidateList) do
        ...
        if needSpamScan then
            ...
            if matchedIndex then
                ...
                elseif cat.Kind == "ENDING" then
                    -- V20.9.9 SHORTEST TRIGGER:
                    -- candidateList is already restricted to the current incoming
                    -- prefix, so the shortest surviving word is the shortest route
                    -- from THIS prompt into the requested spam ending. Examples:
                    -- S -> STYTING for TING, ING -> INGESTING for TING, when those
                    -- are the shortest live/unused Pro entries available.
                    -- Lexical order is only a deterministic tie-break.
...
UpdateList = function(detectedText, requiredLetter)
    ...
    if sortMode == "Godmode" then
        ...
        for priorityIndex, category in ipairs(Config.GodmodePriority) do
            ...
            if allowed then
                -- V20.9.10: for literal spam endings, explicitly select the
                -- shortest live trigger from the CURRENT incoming prefix.
                -- Do not rely on the surrounding sort order: this guarantees
                -- Casual Godmode mirrors Pro Spam's shortest-trigger behavior.
                gd.ShortestPriorityWord = nil
                for _, word in ipairs(matches) do
                    ...
                    if qualifies then
                        if category ~= "__TRAP__"
                            and category ~= "__LOWEST_ENTRY__"
                            and category ~= "__X__" then
                            if not gd.ShortestPriorityWord
                                or #word < #gd.ShortestPriorityWord
                                or (#word == #gd.ShortestPriorityWord and word < gd.ShortestPriorityWord) then
                                gd.ShortestPriorityWord = word
                            end
                        else
                            addDisplay(word)
                            break
                ...
                if gd.ShortestPriorityWord then
                    addDisplay(gd.ShortestPriorityWord)
                end
            end
        end
...
inputConn = UserInputService.InputBegan:Connect(function(input)
    ...
    if input.KeyCode == Enum.KeyCode.F9 then
        ...
        if ShowToast then
            ShowToast(
                env.WordHelperUnbeatable.ProSpamMode
                    and "Pro Spam: ON / SHORTEST triggers / LOWEST ENTRY fallback"
        ...
        if StatusText then
            StatusText.Text = env.WordHelperUnbeatable.ProSpamMode
                and "Pro Spam: ON / shortest choice per ending -> Pro fallback"
                or "Pro Spam Mode: OFF (F9)"
        ...
        if ShowToast then
            ShowToast(
                env.WordHelperUnbeatable.ProSpicyRandom
                    and "Pro Spicy: ON / S2 traps + S3 obscure + S4 3/4 beasts"
