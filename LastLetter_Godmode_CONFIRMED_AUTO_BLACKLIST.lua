...
local function SmartType(targetWord, currentDetected, isCorrection, bypassTurn)
    ...
    local success, err = pcall(function()
        if isCorrection then
            ...
            env.WordHelperBlacklistTracker.BeginAttempt(targetWord)
            PressEnter()
            ...
            if not accepted then
                -- Do not blacklist here. The permanent try-image detector is the
                -- only authority for learning rejected words.
...
-- Learn words rejected after manual typing.
-- Capture only direct game typing, never typing inside WordHelper/chat TextBoxes.
...
env.WordHelperBlacklistTracker.GetRejectionSignals = function()
    local player = Players.LocalPlayer
    local gui = player and player:FindFirstChild("PlayerGui")
    local inGame = gui and gui:FindFirstChild("InGame")
    local signals = {}
    if not inGame then return signals end

    for _, obj in ipairs(inGame:GetDescendants()) do
        if (obj:IsA("TextLabel") or obj:IsA("TextButton")) and obj.Visible then
            local value = tostring(obj.Text or ""):lower()

            -- "Already used" is a valid dictionary word and must NEVER teach the blacklist.
            local alreadyUsed =
                value:find("already used", 1, true)
                or value:find("word used", 1, true)
                or value:find("has been used", 1, true)

            local rejected =
                value:find("invalid word", 1, true)
                or value:find("not a valid word", 1, true)
                or value:find("not in dictionary", 1, true)
                or value:find("word not found", 1, true)
                or value:find("not allowed", 1, true)
                or value:find("banned word", 1, true)
                or value:find("blacklisted", 1, true)
                or value:find("cannot use that word", 1, true)
                or value:find("can't use that word", 1, true)

            if rejected and not alreadyUsed then
                -- Track the exact UI object + text. This lets us distinguish a NEW
                -- rejection from an old rejection label that is still on screen.
                signals[obj] = value
            end
        end
    end
...
env.WordHelperBlacklistTracker.HasFreshRejectionSignal = function(beforeSignals)
    local nowSignals = env.WordHelperBlacklistTracker.GetRejectionSignals()

    for obj, value in pairs(nowSignals) do
        if beforeSignals[obj] ~= value then
            return true
        end
    end
...
env.WordHelperBlacklistTracker.ReadTimerSeconds = function(frame)
    ...
    if seconds and seconds:IsA("TextLabel") then
        return tonumber(tostring(seconds.Text or ""):match("([%d%.]+)"))
    end
    return nil
end

env.WordHelperBlacklistTracker.GetTryImageState = function(frame)
    local state = {}
    local tries = frame and frame:FindFirstChild("Tries")
    if not tries then return state end

    local function relativePath(obj)
        local parts = {}
        local current = obj
        while current and current ~= tries do
    ...
    for _, obj in ipairs(tries:GetDescendants()) do
        if obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
            state[relativePath(obj)] = tostring(obj.Image or "")
        end
    end

    -- In Last Letter the try icons are currently direct children, but include
    -- them explicitly too in case GetDescendants behavior/layout changes.
    for _, obj in ipairs(tries:GetChildren()) do
        if obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
            state[obj.Name] = tostring(obj.Image or "")
        end
    end
...
env.WordHelperBlacklistTracker.GetChangedTryImage = function(beforeState, frame)
    local nowState = env.WordHelperBlacklistTracker.GetTryImageState(frame)

    -- Accepted words can destroy/recreate the Tries icons. That is NOT a
    -- rejection: only an actual image asset change at the same named slot counts.
    for path, beforeImage in pairs(beforeState or {}) do
        local nowImage = nowState[path]
        if nowImage
            and beforeImage ~= ""
            and nowImage ~= ""
            and nowImage ~= beforeImage then
            return path, beforeImage, nowImage
        end
    end
...
env.WordHelperBlacklistTracker.BeginAttempt = function(word)
    word = tostring(word or ""):lower():gsub("[^a-z]", "")

    -- Protection #1: only words that already exist in WordHelper may be learned.
    -- Ordinary typos such as "blakc" therefore never enter the blacklist system.
    if #word < 2 or Blacklist[word] then return end
    if not env.WordHelperBlacklistTracker.IsKnownWord(word) then return end
    ...
    -- Protection #2: the submitted word must match the current required prefix.
    if requiredLetters ~= ""
    ...
    local timerAtSubmit =
        env.WordHelperBlacklistTracker.ReadTimerSeconds(frame)

    -- Protection #3: don't learn from submissions made right on the timeout edge.
    -- The real rejected-word capture changed a try while substantial time remained.
    local eligibleForAutoReject =
        timerAtSubmit ~= nil and timerAtSubmit >= 2.5

    local triesBefore =
        env.WordHelperBlacklistTracker.GetTryImageState(frame)
    local rejectionBefore =
        env.WordHelperBlacklistTracker.GetRejectionSignals()
    ...
    if StatusText then
        StatusText.Text = "Checking submitted word: " .. word
        StatusText.TextColor3 = THEME.SubText
    end

    task.spawn(function()
        -- Poll quickly so the word disappears from suggestions almost immediately
        -- after Last Letter flips one of its Tries image assets.
        local deadline = tick() + 2.25

        while not unloaded
            and env.WordHelperBlacklistTracker.PendingWord == word
            and tick() < deadline do

            -- Existing accepted-word observer is definitive acceptance proof.
            if UsedWords[word] then
                env.WordHelperBlacklistTracker.PendingWord = ""
                return
            end

            -- Keep the old explicit-text detector as a harmless secondary signal,
            -- but the Tries image transition below is the primary proven signal.
            if eligibleForAutoReject
                and env.WordHelperBlacklistTracker.HasFreshRejectionSignal(
                    rejectionBefore
                ) then
                env.WordHelperBlacklistTracker.Add(
                    word,
                    "fresh explicit game rejection"
                )
                env.WordHelperBlacklistTracker.PendingWord = ""
                return
            end

            if eligibleForAutoReject then
                local tryPath, beforeImage, afterImage =
                    env.WordHelperBlacklistTracker.GetChangedTryImage(
                        triesBefore,
                        frame
                    )

                if tryPath then
                    local stillMyTurn, stillRequired = GetTurnInfo(frame)
                    stillRequired =
                        tostring(stillRequired or ""):lower():gsub("[^a-z]", "")
                    local timerNow =
                        env.WordHelperBlacklistTracker.ReadTimerSeconds(frame)

                    -- A real rejection keeps the same player/prefix active and
                    -- changes a try icon before timeout. Accepted words may rebuild
                    -- Tries, but their image assets remain unchanged.
                    if stillMyTurn
                        and stillRequired == requiredLetters
                        and timerNow ~= nil
                        and timerNow >= 0.75
                        ...
                        local added = env.WordHelperBlacklistTracker.Add(
                            word,
                            "confirmed Last Letter try-image rejection"
                        )

                        if added and ShowToast then
                            -- Add() already shows the normal blacklist toast. This
                            -- status detail is useful if debugging is ever needed.
                            if StatusText then
                                StatusText.Text = "Rejected: " .. word .. " (" .. tryPath .. ")"
                                StatusText.TextColor3 = THEME.Warning
                            end
                        end
        ...
        if env.WordHelperBlacklistTracker.PendingWord == word then
            env.WordHelperBlacklistTracker.PendingWord = ""
            if StatusText and not UsedWords[word] then
                if not eligibleForAutoReject then
                    StatusText.Text = "Not learned near timeout: " .. word
                else
                    StatusText.Text = "No rejection confirmed: " .. word
                end
                StatusText.TextColor3 = THEME.SubText
            end
        end
...
env.WordHelperBlacklistTracker.KeyConnection =
    UserInputService.InputBegan:Connect(function(input, processed)
        if unloaded then return end
        -- Last Letter consumes its gameplay keys, so they may arrive with
        -- processed=true. Do not discard them here.
        ...
        -- IMPORTANT: Last Letter itself types through an InGame TextBox.
        -- The old version rejected *every* focused TextBox, which meant manual
        -- gameplay typing was never captured. Allow only the game's own box;
        -- WordHelper search boxes and Roblox chat are still ignored.
        local focused = UserInputService:GetFocusedTextBox()
        ...
        if keyName == "Backspace" then
            local buffer = env.WordHelperBlacklistTracker.TypedBuffer
            env.WordHelperBlacklistTracker.TypedBuffer =
                buffer:sub(1, math.max(0, #buffer - 1))
            return
        end

        if keyName == "Return" or keyName == "KeypadEnter" then
            -- Prefer the actual Last Letter input box over our key buffer. This is
            -- much more reliable when Roblox marks TextBox keystrokes as processed.
            local attemptedWord = env.WordHelperBlacklistTracker.TypedBuffer
            if focused and inGame and focused:IsDescendantOf(inGame) then
                local boxWord = tostring(focused.Text or ""):lower():gsub("[^a-z]", "")
                if #boxWord >= 2 then
                    attemptedWord = boxWord
                end
            end
            env.WordHelperBlacklistTracker.TypedBuffer = ""
            ...
            if #attemptedWord >= 2 then
                if StatusText then
                    StatusText.Text = "Captured attempt: " .. attemptedWord
                    StatusText.TextColor3 = THEME.SubText
                end
        ...
        if #keyName == 1 and keyName:match("%a") then
            local now = tick()
            if now - env.WordHelperBlacklistTracker.LastKeyAt > 3 then
                env.WordHelperBlacklistTracker.TypedBuffer = ""
            end

            env.WordHelperBlacklistTracker.TypedBuffer =
                env.WordHelperBlacklistTracker.TypedBuffer .. keyName:lower()
            env.WordHelperBlacklistTracker.LastKeyAt = now

            -- Clear immediately if the typed prefix cannot be a valid turn entry.
            if requiredLetters ~= ""
                and #env.WordHelperBlacklistTracker.TypedBuffer >= #requiredLetters
                and env.WordHelperBlacklistTracker.TypedBuffer:sub(
                    1,
                    #requiredLetters
                ) ~= requiredLetters then
                env.WordHelperBlacklistTracker.TypedBuffer = ""
            end
        end
    end)
