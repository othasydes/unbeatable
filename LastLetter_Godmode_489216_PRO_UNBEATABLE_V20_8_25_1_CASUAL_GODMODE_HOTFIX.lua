-- V20.8.25.1: HOTFIX - restored working WordHelper; safe stage-aware Casual Godmode scoring
...
GodmodeGetPriorityCategory = function(word)
    ...
    -- Stage is already maintained by the existing tracker before list sorting.
    -- Reading it here is side-effect free, so Casual Godmode cannot break list generation.
    local godStage = math.clamp(tonumber(env.WordHelperUnbeatable.Stage) or 1, 1, 4)

    for index, category in ipairs(Config.GodmodePriority) do
        local stageAllowed = true
        if category == "__TRAP__" then
            stageAllowed = godStage >= 2
        elseif category ~= "__LOWEST_ENTRY__" and category ~= "__X__" then
            stageAllowed = type(category) == "string" and #category <= godStage
        end

        if stageAllowed then
            if category == "__TRAP__" then
                -- Stage 2 uses only personally confirmed Casual 2-letter traps.
                if godStage == 2 then
