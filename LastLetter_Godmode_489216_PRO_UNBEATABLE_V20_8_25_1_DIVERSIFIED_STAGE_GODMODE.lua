-- V20.8.25.1: diversified stage-aware Casual Godmode; working V20.8.24 candidate generation preserved
...
GodmodeGetPriorityCategory = function(word)
    ...
    local currentStage = math.clamp(
        tonumber(env.WordHelperUnbeatable and env.WordHelperUnbeatable.Stage) or 1,
        1, 4
    )

    for index, category in ipairs(Config.GodmodePriority) do
        local allowed = category == "__X__"
            or category == "__LOWEST_ENTRY__"
            or (category == "__TRAP__" and currentStage >= 2)
            or (category ~= "__TRAP__"
                and category ~= "__X__"
                and category ~= "__LOWEST_ENTRY__"
                and type(category) == "string"
                and #category <= currentStage)

        if allowed then
            if category == "__TRAP__" then
                if currentStage == 2 then
                    if #word >= 2 and GodmodeConfirmedTwoLetter[word:sub(-2)] == true then
                        return "TRAP", index, false, nil
                    end
                elseif GodmodeIsTrapWord(word) then
                    return "TRAP", index, false, nil
                end
            elseif category == "__LOWEST_ENTRY__" then
...
UpdateList = function(detectedText, requiredLetter)
    ...
    if #matches > 0 then
        ...
        elseif sortMode == "Shortest" then
            table.sort(matches, function(a, b) return #a < #b end)
        elseif sortMode == "Godmode" then
            -- Keep Casual Godmode on the same authoritative UsedWords stage as Unbeatable.
    ...
    if sortMode == "Godmode" then
        -- V20.8.25.1: display diversity only. `matches` itself is untouched.
        -- First show one representative of every ACTIVE priority category.
        local represented = {}
        local displayed = {}
        local ordinary = {}
        local stageNow = math.clamp(
            tonumber(env.WordHelperUnbeatable and env.WordHelperUnbeatable.Stage) or 1,
            1, 4
        )

        for _, w in ipairs(matches) do
            local _, priorityIndex, exhausted = GodmodeGetPriorityCategory(w)
            if priorityIndex and not exhausted and not represented[priorityIndex] then
                table.insert(displayList, w)
                represented[priorityIndex] = true
                displayed[w] = true
            elseif not priorityIndex then
                table.insert(ordinary, w)
            end
        end

        -- Stage 2 bonus: expose several DIFFERENT confirmed trap endings instead
        -- of allowing one trap/X family to monopolize the page.
        if stageNow == 2 then
            local trapEndSeen = {}
            for _, w in ipairs(matches) do
                local ending = #w >= 2 and w:sub(-2) or ""
                if GodmodeConfirmedTwoLetter[ending]
                    and not trapEndSeen[ending]
                    and not displayed[w]
                    and #displayList < 18 then
                    table.insert(displayList, w)
                    displayed[w] = true
                    trapEndSeen[ending] = true
                end
            end
        end

        -- Short "be unique" segment: up to 6 ordinary words. Sort shortest first,
        -- then shuffle only the first 12 so the fallback stays quick to type.
        table.sort(ordinary, function(a,b)
            if #a == #b then return a < b end
            return #a < #b
        end)
        local shortCount = math.min(12, #ordinary)
        for i = shortCount, 2, -1 do
            local j = math.random(i)
            ordinary[i], ordinary[j] = ordinary[j], ordinary[i]
        end
        for i = 1, math.min(6, shortCount) do
            if not displayed[ordinary[i]] and #displayList < maxDisplay then
                table.insert(displayList, ordinary[i])
                displayed[ordinary[i]] = true
            end
        end

        -- Fill remaining rows from the original strategic ordering.
        for _, w in ipairs(matches) do
            if #displayList >= maxDisplay then break end
            if not displayed[w] then
                table.insert(displayList, w)
                displayed[w] = true
            end
        end
