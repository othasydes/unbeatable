...
env.WordHelperBlacklistTracker.BeginAttempt = function(word)
    ...
    -- Protection #1: typos / unknown strings are ignored completely.
    ...
    -- Protection #2: it must actually be a legal candidate for the current turn.
    if requiredLetters ~= ""
    ...
    -- Snapshot the game immediately before Enter is processed.
    -- These are used as a fallback when Last Letter rejects silently without
    -- exposing a useful invalid-word TextLabel.
    local rejectionBefore =
        env.WordHelperBlacklistTracker.GetRejectionSignals()
    local currentWordBefore = select(1, GetCurrentGameWord(frame)) or ""
    local submitBox = GetGameTextBox()
    local submitBoxText = submitBox and tostring(submitBox.Text or ""):lower() or ""
    ...
    task.spawn(function()
        -- Fast polling because Last Letter gameplay is time-sensitive.
        local started = tick()
        local deadline = started + 1.40

        while not unloaded
            and env.WordHelperBlacklistTracker.PendingWord == word
            and tick() < deadline do

            -- Strong acceptance proof: the existing observer saw the game accept it.
            if UsedWords[word] then
                env.WordHelperBlacklistTracker.PendingWord = ""
                return
            end

            -- Strong rejection proof #1: the game displayed a NEW invalid-word signal.
            if env.WordHelperBlacklistTracker.HasFreshRejectionSignal(
                rejectionBefore
            ) then
                env.WordHelperBlacklistTracker.Add(
                    word,
            ...
            -- Strong rejection proof #2: silent rejection. Last Letter can reject a
            -- submission without leaving a useful error label. After a short grace
            -- period, if our turn + required prefix + displayed current word have
            -- all remained unchanged, and the game's input box either still holds
            -- the attempted word or has been cleared by the game, treat that as a
            -- rejection. Unknown/mistyped words never reach BeginAttempt(), so they
            -- still cannot poison the blacklist.
            if (tick() - started) >= 0.45 then
                local nowMyTurn, nowRequired = GetTurnInfo(frame)
                nowRequired = tostring(nowRequired or ""):lower():gsub("[^a-z]", "")
                local currentWordNow = select(1, GetCurrentGameWord(frame)) or ""
                local gameBoxNow = GetGameTextBox()
                local boxTextNow = gameBoxNow and tostring(gameBoxNow.Text or ""):lower() or ""
                local boxLooksRejected =
                    boxTextNow == ""
                    or boxTextNow == word
                    or boxTextNow == submitBoxText

                if nowMyTurn
                    and nowRequired == requiredLetters
                    and currentWordNow == currentWordBefore
                    and boxLooksRejected
                    and not UsedWords[word] then
                    env.WordHelperBlacklistTracker.Add(
                        word,
                        "silent game rejection"
                    )
                    env.WordHelperBlacklistTracker.PendingWord = ""
                    return
                end
...
env.WordHelperUIDebugger = env.WordHelperUIDebugger or {
    Active = false,
    FileName = "WordHelper_UI_Debug.txt",
    Duration = 8.0,
    PollRate = 0.025
}
...
env.WordHelperUIDebugger.Start = function()
    if env.WordHelperUIDebugger.Active then
        if ShowToast then
            ShowToast("F7 UI capture is already running", "warning")
        end
        return
    end
    ...
    if ShowToast then
        ShowToast("F7 UI capture started - submit rejected word now", "success")
    end
    if StatusText then
        StatusText.Text = "F7 UI capture running..."
        StatusText.TextColor3 = THEME.Warning
    end
    ...
    task.spawn(function()
        ...
        table.insert(lines, "WordHelper Last Letter UI Rejection Capture")
        ...
        if ok then
            if ShowToast then
                ShowToast("UI capture saved: " .. env.WordHelperUIDebugger.FileName, "success")
            end
            if StatusText then
                StatusText.Text = "F7 capture saved (" .. tostring(eventCount) .. " changes)"
                StatusText.TextColor3 = THEME.Success
            end
        else
            if ShowToast then
