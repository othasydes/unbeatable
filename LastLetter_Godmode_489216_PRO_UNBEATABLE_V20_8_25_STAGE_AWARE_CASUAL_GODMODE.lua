-- V20.8.25: stage-aware diversified Casual Godmode + confirmed Casual Stage-2 traps
...
GodmodeIsStageTrapWord = function(word)
    ...
    -- Stage 1 cannot return a 2+ letter trap.
    if stage <= 1 then return false end

    -- Stage 2: use ONLY the Casual-server pairs the user has personally confirmed.
    if stage == 2 then
        return #word >= 2 and GodmodeConfirmedTwoLetter[word:sub(-2)] == true
    end

    -- Stage 3/4 retain the established manual trap-word catalogue. Confirmed
    -- 2-letter pairs are NOT promoted here because the game will first attempt
    -- the longer stage-length suffix.
    return TrapWordPriority[word] == true or CustomTrapWords[word] == true
end

GodmodePriorityAvailableAtStage = function(category)
    env.WordHelperUnbeatable.SyncStageFromUsedWords()
    local stage = math.clamp(tonumber(env.WordHelperUnbeatable.Stage) or 1, 1, 4)
    if category == "__TRAP__" then return stage >= 2 end
    if category == "__X__" then return true end
    if category == "__LOWEST_ENTRY__" then return true end
    return type(category) == "string" and #category <= stage
end
...
GodmodeMatchesConfiguredPriority = function(word)
    for index, category in ipairs(Config.GodmodePriority) do
        if not GodmodePriorityAvailableAtStage(category) then
            -- Impossible at this stage: continue to the next configured priority.
        elseif category == "__TRAP__" then
            if GodmodeIsStageTrapWord(word) then
...
GodmodeGetPriorityCategory = function(word)
    ...
    for index, category in ipairs(Config.GodmodePriority) do
        if not GodmodePriorityAvailableAtStage(category) then
            -- Skip priorities the current game's return length cannot trigger.
        elseif category == "__TRAP__" then
            if GodmodeIsStageTrapWord(word) then
...
GodmodePrepareLowestEntry = function(exacts, bucket, prefix, tryFallbackLengths)
    ...
    for _, word in ipairs(exacts) do
        for i = 1, lowestIndex - 1 do
            ...
            if not GodmodePriorityAvailableAtStage(category) then
                -- Ignore priorities impossible at this stage.
            elseif category == "__TRAP__" then
                if GodmodeIsStageTrapWord(word) then return exacts end
            elseif category == "__X__" then
...
UpdateList = function(detectedText, requiredLetter)
    ...
    if #matches > 0 then
        ...
        elseif sortMode == "Godmode" then
            ...
            -- V20.8.25: diversified Casual Godmode display.
            -- Keep ONE best word from every priority category that can actually
            -- trigger at the current stage, then append a small shuffled pool of
            -- short ordinary words for optional variety.
            local diversified, represented, ordinary = {}, {}, {}
            for _, w in ipairs(matches) do
                local category, categoryIndex, exhausted = GodmodeGetPriorityCategory(w)
                if categoryIndex and not exhausted and not represented[categoryIndex] then
                    diversified[#diversified + 1] = w
                    represented[categoryIndex] = true
                elseif not categoryIndex then
                    ordinary[#ordinary + 1] = w
                end
            end

            table.sort(diversified, function(a, b)
                return (godScoreCache[a] or 0) > (godScoreCache[b] or 0)
            end)
            ...
            -- Small uniqueness segment: randomize only among the shortest 12
            -- ordinary candidates, then expose up to 6 after strategic choices.
            local shortPoolLimit = math.min(12, #ordinary)
            for i = shortPoolLimit, 2, -1 do
                local j = math.random(1, i)
                ordinary[i], ordinary[j] = ordinary[j], ordinary[i]
            end
            for i = 1, math.min(6, shortPoolLimit) do
                diversified[#diversified + 1] = ordinary[i]
            end

            -- Preserve full fallback depth after the front-page choices.
            local already = {}
            for _, w in ipairs(diversified) do already[w] = true end
            for _, w in ipairs(matches) do
                if not already[w] then diversified[#diversified + 1] = w end
            end
            matches = diversified
