...
env.WordHelperDiscovery.ObserveFrame = function(detectedWord, censored, typeText, now, frame)
    ...
    -- IMPORTANT: discovery is committed ONLY after two independent acceptance
    -- signals occur together:
    --   1) Last Letter produces a positive WPM result, and
    --   2) the Type prompt moves to the next player shortly afterwards.
    -- A changing/partial CurrentWord by itself is never considered accepted.
    -- This prevents fragments such as "inescapabl" / "lafayett" from being
    -- learned while somebody is still typing.
    ...
    elseif typeText ~= env.WordHelperDiscovery.LastTypeText then
        ...
        -- IMPORTANT: commit the candidate FROZEN at the WPM acceptance pulse.
        -- Last Letter animates/deletes CurrentWord tiles after acceptance, and those
        -- transient fragments must never replace the submitted word before Type changes.
        -- Commit the LONGEST valid candidate observed from the acceptance pulse
        -- until the turn actually changes. Last Letter can update WPM a frame before
        -- the final typed tile appears, so the first snapshot may be one letter short.
        local candidate = tostring(env.WordHelperDiscovery.AcceptanceBestCandidate or "")
        local candidateAt = tonumber(env.WordHelperDiscovery.AcceptanceBestCandidateAt or 0) or 0
        local wrongAt = tonumber(env.WordHelperDiscovery.LastWrongAt or 0) or 0

        if pulseAt > 0
            and (now - pulseAt) <= 0.90
            and (now - wrongAt) > 0.75
            and #candidate >= 2
            and (now - candidateAt) <= 1.50
            and env.WordHelperDiscovery.AcceptanceBestCandidateValid == true then
            env.WordHelperDiscovery.ConsiderAccepted(candidate, "frozen WPM snapshot + turn change")
        end

        -- The previous submission is over. Clear every capture field BEFORE
        -- observing the next player's starting tiles so submissions cannot bleed
        -- into one another.
        env.WordHelperDiscovery.CurrentCandidate = ""
        env.WordHelperDiscovery.CurrentCandidateAt = 0
        env.WordHelperDiscovery.FullCandidate = ""
        env.WordHelperDiscovery.LastVisibleSuffix = ""
        env.WordHelperDiscovery.CandidateTruncated = false
        env.WordHelperDiscovery.ReconstructionValid = true
        env.WordHelperDiscovery.AcceptancePulseAt = 0
        env.WordHelperDiscovery.AcceptanceCandidate = ""
        env.WordHelperDiscovery.AcceptanceCandidateAt = 0
        env.WordHelperDiscovery.AcceptanceCandidateValid = false
        env.WordHelperDiscovery.AcceptanceBestCandidate = ""
        env.WordHelperDiscovery.AcceptanceBestCandidateAt = 0
        env.WordHelperDiscovery.AcceptanceBestCandidateValid = false
        env.WordHelperDiscovery.LastTypeText = typeText
    end

    -- Last Letter truncates long typed words visually with "..". Keep building a
    -- full pre-submit candidate while typing, but NEVER learn it at this stage.
    local raw = tostring(detectedWord or ""):lower():gsub("%s+", "")
    local visible = raw:gsub("[^a-z]", "")
    local hasTruncation = raw:find("..", 1, true) ~= nil
        or raw:find("…", 1, true) ~= nil

    if #visible >= 2 then
        if not hasTruncation then
            env.WordHelperDiscovery.FullCandidate = visible
            env.WordHelperDiscovery.CurrentCandidate = visible
            env.WordHelperDiscovery.LastVisibleSuffix = visible
            env.WordHelperDiscovery.CandidateTruncated = false
            env.WordHelperDiscovery.ReconstructionValid = true
            env.WordHelperDiscovery.CurrentCandidateAt = now
            env.WordHelperDiscovery.CurrentCandidateType = typeText
        else
            env.WordHelperDiscovery.CandidateTruncated = true

            local full = tostring(env.WordHelperDiscovery.FullCandidate or "")
            local reconstructed = nil

            if #full >= 2 then
                if #visible <= #full and full:sub(-#visible) == visible then
                    reconstructed = full
                else
                    local maxOverlap = math.min(#full, #visible)
                    local overlap = 0
                    for size = maxOverlap, 2, -1 do
                        if full:sub(-size) == visible:sub(1, size) then
                            overlap = size
                            break
                        end
                    end
                    if overlap >= 2 then
                        reconstructed = full .. visible:sub(overlap + 1)
                    end
                end
            end

            if reconstructed and #reconstructed >= #visible then
                env.WordHelperDiscovery.FullCandidate = reconstructed
                env.WordHelperDiscovery.CurrentCandidate = reconstructed
                env.WordHelperDiscovery.LastVisibleSuffix = visible
                env.WordHelperDiscovery.ReconstructionValid = true
                env.WordHelperDiscovery.CurrentCandidateAt = now
                env.WordHelperDiscovery.CurrentCandidateType = typeText
            else
                -- Never substitute a visible suffix for a missing hidden prefix.
                env.WordHelperDiscovery.ReconstructionValid = false
                env.WordHelperDiscovery.LastVisibleSuffix = visible
            end
        end
    end

    -- After a positive WPM pulse, keep the longest valid candidate seen until
    -- the Type prompt changes. This catches a final tile that renders a frame late
    -- (e.g. elephan -> elephant) while ignoring the later delete/shrink animation.
    local activePulseAt = tonumber(env.WordHelperDiscovery.AcceptancePulseAt or 0) or 0
    if activePulseAt > 0 and (now - activePulseAt) <= 0.90 then
        local liveCandidate = tostring(env.WordHelperDiscovery.CurrentCandidate or "")
        local liveValid = #liveCandidate >= 2
            and (not env.WordHelperDiscovery.CandidateTruncated
                or env.WordHelperDiscovery.ReconstructionValid == true)
        if liveValid then
            local best = tostring(env.WordHelperDiscovery.AcceptanceBestCandidate or "")
            if #liveCandidate > #best then
                env.WordHelperDiscovery.AcceptanceBestCandidate = liveCandidate
                env.WordHelperDiscovery.AcceptanceBestCandidateAt =
                    tonumber(env.WordHelperDiscovery.CurrentCandidateAt or now) or now
                env.WordHelperDiscovery.AcceptanceBestCandidateValid = true
            end
        end
    end

    -- WPM is only an acceptance *pulse*. Do not add the candidate here. We wait
    -- for the subsequent turn-prompt change above, which distinguishes a real
    -- accepted submission from an in-progress partial word.
    ...
    if wpmText ~= env.WordHelperDiscovery.LastWPMText then
        ...
        if amount and amount > 0 then
            ...
            -- Freeze the submitted word NOW. Do not read CurrentCandidate later,
            -- because the acceptance animation mutates/removes the visible tiles.
            env.WordHelperDiscovery.AcceptanceCandidate =
                tostring(env.WordHelperDiscovery.CurrentCandidate or "")
            env.WordHelperDiscovery.AcceptanceCandidateAt =
                tonumber(env.WordHelperDiscovery.CurrentCandidateAt or now) or now
            env.WordHelperDiscovery.AcceptanceCandidateValid =
                #env.WordHelperDiscovery.AcceptanceCandidate >= 2
                and (not env.WordHelperDiscovery.CandidateTruncated
                    or env.WordHelperDiscovery.ReconstructionValid == true)

            -- Seed the post-acceptance longest-candidate window with the current
            -- snapshot, then allow later frames to extend it before the turn changes.
            env.WordHelperDiscovery.AcceptanceBestCandidate =
                tostring(env.WordHelperDiscovery.AcceptanceCandidate or "")
            env.WordHelperDiscovery.AcceptanceBestCandidateAt =
                tonumber(env.WordHelperDiscovery.AcceptanceCandidateAt or now) or now
            env.WordHelperDiscovery.AcceptanceBestCandidateValid =
                env.WordHelperDiscovery.AcceptanceCandidateValid == true
        end
    end
end
