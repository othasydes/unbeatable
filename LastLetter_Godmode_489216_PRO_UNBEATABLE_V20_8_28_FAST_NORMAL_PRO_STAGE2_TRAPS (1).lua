-- V20.8.28: fast Normal Pro shortlist + expanded Stage-2 Pro traps; F9 V20.8.27 preserved
...
env.WordHelperUnbeatable.SelectTopProCandidates = function(candidateList)
    ...
    -- V20.8.28 NORMAL PRO FAST PATH.
    -- Keep the canonical Normal-Pro comparator/analysis below, but feed it a bounded
    -- strategic shortlist rather than doing full reply-tree analysis on every match.
    ...
    -- At Stage 2, explicitly seed every available confirmed Pro trap ending.
    -- Pro-only spicy traps are seeded first so RZ/VP/MZ/GD/WD/PM cannot be crowded
    -- out by ordinary candidates.
    if stage == 2 then
        for _, w in ipairs(originalCandidateList) do
            if #w >= 2 then
                local ending = w:sub(-2)
                if env.WordHelperUnbeatable.ProGodmodePriorityTwoLetterTraps[ending] then
                    if not spicySeen[ending] then
                        spicySeen[ending] = true
                        spicyStageTwo[#spicyStageTwo + 1] = w
                    end
                elseif env.WordHelperUnbeatable.ProGodmodeTwoLetterTraps[ending] then
                    if not regularSeen[ending] then
                        regularSeen[ending] = true
    ...
    -- Broad but bounded sample for the existing lowest-reply / self-solve logic.
    -- 180 keeps substantially more strategic depth than F9's 72 while eliminating
    -- the old unbounded per-prefix analysis responsible for the freeze.
    for _, w in ipairs(originalCandidateList) do
        addShort(w)
