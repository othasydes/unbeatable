...
local function GetGameTextBox()
    ...
    local focused = UserInputService:GetFocusedTextBox()
    -- A WordHelper textbox is never a gameplay input.
    if focused and ScreenGui and focused:IsDescendantOf(ScreenGui) then
        return nil
    end
    return focused
end
...
local function SmartType(targetWord, currentDetected, isCorrection, bypassTurn)
    ...
    if not targetBox then
        -- Never spray virtual keystrokes into whichever UI field happens to be focused.
        isTyping = false
        isAutoPlayScheduled = false
        StatusText.Text = "Game input not found"
        StatusText.TextColor3 = THEME.Warning
        return
    end
    ...
    local success, err = pcall(function()
        if isCorrection then
            ...
            if not accepted then
                ...
                local focused = UserInputService:GetFocusedTextBox()
                if focused and ScreenGui and focused:IsDescendantOf(ScreenGui) then
                    -- Never clear WordHelper's own search/settings fields.
                elseif focused and focused:IsDescendantOf(game) and focused.TextEditable then
                    focused.Text = ""
                else
...
UpdateList = function(detectedText, requiredLetter)
    ...
    local function CollectMatches(prefix, tryFallbackLengths)
        ...
        if bucket then
            local checkWord = function(w)
                ...
                -- Manual browsing is independent from gameplay suffix/length filters.
                if not manualSearch and suffixMode ~= "" and w:sub(-#suffixMode) ~= suffixMode then return end

                local isLengthMatch = true
                if not manualSearch and not tryFallbackLengths and lengthMode > 0 then
                    isLengthMatch = (#w == lengthMode)
                elseif not manualSearch and tryFallbackLengths and lengthMode > 0 then
                    isLengthMatch = true
                end
                    ...
                    for i = startIndex, #bucket do
                        ...
                        elseif sortMode == "Godmode" then
                            ...
                            if canAddPriority
                                ...
                                and (manualSearch or suffixMode == "" or w:sub(-#suffixMode) == suffixMode)
                                and (manualSearch or lengthMode == 0 or tryFallbackLengths or #w == lengthMode) then
                                table.insert(exacts, w)
                                seenExact[w] = true
    ...
    if not manualSearch and #exacts == 0 and lengthMode > 0 then
    ...
    if manualSearch then
        -- Search box is a browser only; never let it drive autoplay.
        currentBestMatch = nil
    elseif #matches > 0 and not isBacktracked then
...
runConn = RunService.RenderStepped:Connect(function()
    local success, err = pcall(function()
        ...
        local helperFocused = focusedBox and ScreenGui and focusedBox:IsDescendantOf(ScreenGui)
        if autoPlay and not helperFocused and not isTyping and not isAutoPlayScheduled and currentBestMatch and detected == lastDetected then
