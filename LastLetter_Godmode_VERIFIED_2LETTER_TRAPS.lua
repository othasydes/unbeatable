...
-- Collector-verified 2-letter trap sender words.
-- Only prefixes whose casual collector branch is explicitly COMPLETE are included.
-- The value is the 2-letter trap prefix; keys are collected words that can send it.
WordHelperVerified2LTrapSenders = {
    -- CM: cmdg, cmdr, cmon
    ["lcm"] = "cm",
    -- DC self-solve: dc, dca, dcb, dcbname
    ["adc"] = "dc",
    ["dc"] = "dc",
    ["idc"] = "dc",
    -- DN: dna, dni, dnieper
    ...
    ["wadt"] = "dt",
    ["zuurveldt"] = "dt",
    -- GM self-solve: gm, gmelina, gmelinite, gmelinites
    ["apophthegm"] = "gm",
    ["apothegm"] = "gm",
    ["borborygm"] = "gm",
    ["eclegm"] = "gm",
    ...
    ["phlegm"] = "gm",
    ["phoenigm"] = "gm",
    -- LV: lvalue, lvalues, lvov
    ["galv"] = "lv",
    -- MD: mdewakanton, mdnt, mdse
    ["ftncmd"] = "md",
    ["mtscmd"] = "md",
    ["omd"] = "md",
    -- MZ: mzee, mzees, mzungus
    ["skramz"] = "mz",
    -- NR: nrarucu, nritta, nritya
    ["lnr"] = "nr",
    -- RF: rfb, rfound, rfree
    ["arf"] = "rf",
    ["barf"] = "rf",
    ...
    ["wirl"] = "rl",
    ["yarl"] = "rl",
    -- TV self-solve: tv, tveitite, tveitites, tvorog
    ["tv"] = "tv",
    ["wtv"] = "tv",
    -- UW: uwa, uwas, uwu
    ["kabeljouw"] = "uw",
    ...
    ["paauw"] = "uw",
    ["vrouw"] = "uw",
    -- VH: vhavenda, vhost, vhosts
    ["jhvh"] = "vh",
    -- YW self-solve: yw, ywca, ywis, ywroke
    ["yw"] = "yw",
}

-- Ensure every verified sender is available even if the fetched base dictionary lacks it.
-- Kept in a function so temporary variables do not consume main-chunk Luau registers.
WordHelperInstallVerified2LTrapSenders = function()
    for senderWord in pairs(WordHelperVerified2LTrapSenders) do
        if not WordHelperKnownWords[senderWord] then
            WordHelperKnownWords[senderWord] = true
            if SeenWords then SeenWords[senderWord] = true end
            table.insert(Words, senderWord)

            local firstLetter = senderWord:sub(1, 1)
            if firstLetter == "" then firstLetter = "#" end
            Buckets[firstLetter] = Buckets[firstLetter] or {}
            table.insert(Buckets[firstLetter], senderWord)
        end
    end
end

WordHelperInstallVerified2LTrapSenders()
...
-- Promote all completed-branch 2-letter trap senders to GodMode's absolute TRAP tier.
WordHelperMarkVerified2LTrapSenders = function()
    for senderWord in pairs(WordHelperVerified2LTrapSenders or {}) do
        TrapWordPriority[senderWord] = true
    end
end
WordHelperMarkVerified2LTrapSenders()
