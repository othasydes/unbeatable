-- V20.8.29: full-pool Pro trap discovery + shortest triggers + fast strategic analysis; F9 preserved
...
env.WordHelperUnbeatable.SelectTopProCandidates = function(candidateList)
    ...
    -- V20.8.29 NORMAL PRO FULL-COVERAGE DISCOVERY.
    -- Cheaply inspect EVERY valid candidate for its ACTUAL stage-legal returned
    -- prefix, but retain only the shortest trigger for each distinct return.
    -- Expensive forced-reply analysis remains bounded later.
    local originalCandidateList = candidateList
    local stage = math.clamp(tonumber(env.WordHelperUnbeatable.Stage) or 1, 1, 4)
    local shortlist, shortlistSeen = {}, {}
    local spicyStageTwo, regularStageTwo = {}, {}
    ...
    -- FULL candidate-pool discovery. GetProBaseReplyInfo is O(1) against the
    -- prefix indexes/counters (plus the tiny Pro overlay), so this avoids the
    -- expensive forced-response scans that caused the old freeze.
    ...
    -- Turn the complete return map into a strategic ordering. This means obscure
    -- Stage-3/4 traps and X endings cannot disappear merely because they occur late
    -- alphabetically. One shortest trigger represents each actual returned prefix.
    local discovered = {}
    ...
    -- Stage 2: explicitly promote the complete confirmed Pro trap bank, using the
    -- SHORTEST trigger discovered for each return. Requested Pro-only traps first.
    if stage == 2 then
    ...
    -- Preserve a globally-discovered X trigger regardless of alphabetical position.
    addShort(xBest)

    -- Seed all perfect/near-perfect returns first across the FULL pool, then fill
    -- with the strongest remaining distinct returns. The expensive canonical Pro
    -- comparator below sees at most 180 representatives.
    for _, row in ipairs(discovered) do
    ...
    -- Absolute safety if discovery is unusually sparse.
