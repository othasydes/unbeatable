...
-- Last Letter's confirmed rejection sound from the UI/sound captures:
-- SoundService.Game.Wrong, SoundId rbxassetid://4612384231.
-- Store these on the tracker table instead of top-level locals so this very large
-- script stays below Luau's 200-local register limit.
env.WordHelperBlacklistTracker.SoundService = game:GetService("SoundService")
env.WordHelperBlacklistTracker.GameSounds =
    env.WordHelperBlacklistTracker.SoundService:FindFirstChild("Game")
env.WordHelperBlacklistTracker.WrongSound =
    env.WordHelperBlacklistTracker.GameSounds
    and env.WordHelperBlacklistTracker.GameSounds:FindFirstChild("Wrong")

if env.WordHelperBlacklistTracker.WrongSoundConnection then
    pcall(function()
        env.WordHelperBlacklistTracker.WrongSoundConnection:Disconnect()
    end)
    env.WordHelperBlacklistTracker.WrongSoundConnection = nil
end

if env.WordHelperBlacklistTracker.WrongSound
    and env.WordHelperBlacklistTracker.WrongSound:IsA("Sound") then
    env.WordHelperBlacklistTracker.WrongSoundConnection =
        env.WordHelperBlacklistTracker.WrongSound.Played:Connect(function()
            if unloaded then return end
            ...
            -- Rejection must happen while it is still our turn. Accepted words
            -- rotate the turn, and Wrong sounds from unrelated players are ignored.
            ...
            -- The rejection captures proved CurrentWord still contains the failed
            -- submission when SoundService.Game.Wrong fires. Read it directly.
            local candidate = select(1, GetCurrentGameWord(frame)) or ""
            candidate = tostring(candidate):lower():gsub("[^a-z]", "")

            -- If Last Letter has already started changing the tiles, use the
            -- continuously maintained pre-submit cache, but only when very fresh.
            local cached = tostring(
                env.WordHelperBlacklistTracker.LastPreSubmitWord or ""
            ):lower():gsub("[^a-z]", "")
            local cachedAt = tonumber(
                env.WordHelperBlacklistTracker.LastPreSubmitAt or 0
            ) or 0

            if (#candidate < 2 or candidate == nowRequired)
                and #cached >= 2
                and (tick() - cachedAt) <= 1.5 then
                candidate = cached
            end

            -- Typo protection: only words that already exist in WordHelper's
            -- dictionary are eligible for automatic rejection learning.
            if #candidate < 2 then return end
            if Blacklist[candidate] then return end
            if UsedWords[candidate] then return end
            if not env.WordHelperBlacklistTracker.IsKnownWord(candidate) then
                return
            end

            if nowRequired ~= ""
                and candidate:sub(1, #nowRequired) ~= nowRequired then
                return
            end

            local added = env.WordHelperBlacklistTracker.Add(
                candidate,
                "confirmed SoundService.Game.Wrong rejection"
            )

            if added then
                -- Show this here rather than relying on the older Add() closure,
                -- which was defined before the script-local ShowToast function.
                if ShowToast then
                    ShowToast(
                        "Rejected word permanently blacklisted: " .. candidate,
                        "warning"
                    )
                end
                if StatusText then
    ...
    if StatusText then
        StatusText.Text = "Warning: rejection sound not found"
        StatusText.TextColor3 = THEME.Warning
    end
end
