...
--
-- CurrentWord is a LIVE typing UI, not a reliable final-word value. Last Letter can
-- truncate long words, animate/remove tiles after acceptance, and players can type,
-- backspace and correct themselves very quickly. Discovery therefore maintains its
-- own internal character buffer from tile EVENTS instead of learning snapshots.
--
-- Important behavior:
--   * numeric CurrentWord tile + Letter text -> set that exact character index
--   * tile renamed to "Deleted"             -> treat as a real backspace
--   * ordinary tile removal                 -> IGNORE (truncation/accept animation)
--   * WPM pulse                              -> arm acceptance, but do not learn yet
--   * Type prompt changes                    -> confirm the armed internal buffer
--   * ambiguous event history                -> skip the word, never guess
...
env.WordHelperDiscovery.RecorderAddConnection = function(connection)
    ...
    env.WordHelperDiscovery.RecorderConnections =
        env.WordHelperDiscovery.RecorderConnections or {}
    table.insert(env.WordHelperDiscovery.RecorderConnections, connection)
end

env.WordHelperDiscovery.RecorderRebuild = function(now)
    local slots = env.WordHelperDiscovery.RecorderSlots or {}
    local maxIndex = tonumber(env.WordHelperDiscovery.RecorderMaxIndex or 0) or 0
    local pieces = {}

    for index = 1, maxIndex do
        local value = slots[index]
        if value == nil then
            -- A missing interior character means we cannot prove the exact word.
            -- Fail closed rather than creating a fragment/misspelling.
    ...
    local buffer = table.concat(pieces)
    env.WordHelperDiscovery.RecorderBuffer = buffer
    env.WordHelperDiscovery.RecorderBufferAt = now or tick()

    -- If acceptance is already armed, only let the accepted snapshot grow by a
    -- clean prefix extension. It can never shrink during the post-accept animation.
    local pulseAt = tonumber(env.WordHelperDiscovery.AcceptancePulseAt or 0) or 0
    if pulseAt > 0 and (tick() - pulseAt) <= 1.25
        and env.WordHelperDiscovery.RecorderAmbiguous ~= true then
        local accepted = tostring(env.WordHelperDiscovery.AcceptanceEventBuffer or "")
        if accepted == "" then
            env.WordHelperDiscovery.AcceptanceEventBuffer = buffer
            env.WordHelperDiscovery.AcceptanceEventBufferAt = now or tick()
        elseif #buffer > #accepted and buffer:sub(1, #accepted) == accepted then
            env.WordHelperDiscovery.AcceptanceEventBuffer = buffer
            env.WordHelperDiscovery.AcceptanceEventBufferAt = now or tick()
        elseif #buffer < #accepted and accepted:sub(1, #buffer) == buffer then
            -- Ignore shrinkage after acceptance. This is normally UI animation.
        elseif buffer ~= accepted then
            -- A non-prefix mutation after acceptance is not safe to infer.
...
env.WordHelperDiscovery.RecorderSetSlot = function(index, value, now)
    if env.WordHelperDiscovery.RecorderReady ~= true then return end
    index = tonumber(index)
    if not index or index < 1 or index > 80 then return end

    value = tostring(value or ""):lower():gsub("[^a-z]", "")
    if #value < 1 then return end
    value = value:sub(1, 1)

    env.WordHelperDiscovery.RecorderSlots =
        env.WordHelperDiscovery.RecorderSlots or {}
    local slots = env.WordHelperDiscovery.RecorderSlots
    local maxIndex = tonumber(env.WordHelperDiscovery.RecorderMaxIndex or 0) or 0

    -- Numeric tile indices should advance contiguously. A jump means we began
    -- observing after some hidden letters were already lost; skip that turn.
    if maxIndex > 0 and index > (maxIndex + 1) then
        env.WordHelperDiscovery.RecorderAmbiguous = true
        return
    end
    ...
    if index > maxIndex then
        env.WordHelperDiscovery.RecorderMaxIndex = index
    end
    env.WordHelperDiscovery.RecorderRebuild(now)
end

env.WordHelperDiscovery.RecorderDeleteSlot = function(index, now)
    if env.WordHelperDiscovery.RecorderReady ~= true then return end
    index = tonumber(index)
    ...
    -- Backspace should delete the current last logical character. If Last Letter
    -- reports deletion of an interior index, the history is ambiguous; do not learn.
    if index ~= maxIndex then
        env.WordHelperDiscovery.RecorderAmbiguous = true
        return
    end
    ...
    slots[index] = nil
    maxIndex = maxIndex - 1
    while maxIndex > 0 and slots[maxIndex] == nil do
        maxIndex = maxIndex - 1
    end
    env.WordHelperDiscovery.RecorderMaxIndex = maxIndex
    env.WordHelperDiscovery.RecorderRebuild(now)
end
...
env.WordHelperDiscovery.RecorderTrackChild = function(child)
    ...
    env.WordHelperDiscovery.RecorderTracked =
        env.WordHelperDiscovery.RecorderTracked or {}
    if env.WordHelperDiscovery.RecorderTracked[child] then return end

    -- IMPORTANT: child.Name is only a DISPLAY index. Last Letter renumbers visible
    -- tiles when long words scroll/truncate, so after initial seeding we never use
    -- later numeric renames to decide logical character positions.
    local state = {
        LogicalIndex = nil,
        Letter = nil,
        LastText = "",
    ...
    env.WordHelperDiscovery.RecorderTracked[child] = state

    local function pushLetter(isInitialBind)
        if not state.Letter or not state.Letter.Parent then return end
        local clean = tostring(state.Letter.Text or ""):lower():gsub("[^a-z]", "")
        if #clean < 1 then return end
        clean = clean:sub(1, 1)

        -- During the seed pass only, numeric names map the already-visible prefix.
        if env.WordHelperDiscovery.RecorderSeeding == true then
            local numericIndex = tonumber(child.Name)
            if numericIndex then
                state.LogicalIndex = numericIndex
                state.Seeded = true
                state.LastText = clean
                env.WordHelperDiscovery.RecorderSetSlot(numericIndex, clean, tick())
            end
            return
        end

        if env.WordHelperDiscovery.RecorderReady ~= true then
            state.LastText = clean
            return
        end

        if not state.LogicalIndex then
            -- A genuinely new tile after seeding represents the next typed letter.
            local nextIndex = (tonumber(env.WordHelperDiscovery.RecorderMaxIndex or 0) or 0) + 1
            state.LogicalIndex = nextIndex
            state.LastText = clean
            env.WordHelperDiscovery.RecorderSetSlot(nextIndex, clean, tick())
            return
        end

        if state.LastText == "" then
            state.LastText = clean
            env.WordHelperDiscovery.RecorderSetSlot(state.LogicalIndex, clean, tick())
            return
        end

        if clean ~= state.LastText then
            -- Reusing/changing an existing visible tile is ambiguous: it can be a
            -- correction, scroll recycling, or animation. Never guess. Skip this turn.
            env.WordHelperDiscovery.RecorderAmbiguous = true
            env.WordHelperDiscovery.AcceptanceEventValid = false
            state.LastText = clean
        end
    ...
    local function bindLetter()
        if state.Letter and state.Letter.Parent then return end
        local letter = child:FindFirstChild("Letter")
        if not (letter and letter:IsA("TextLabel")) then return end
        state.Letter = letter
        pushLetter(true)
        env.WordHelperDiscovery.RecorderAddConnection(
            letter:GetPropertyChangedSignal("Text"):Connect(function()
                pushLetter(false)
            end)
        )
    end
    ...
    env.WordHelperDiscovery.RecorderAddConnection(
        child.DescendantAdded:Connect(function(descendant)
            if descendant.Name == "Letter" and descendant:IsA("TextLabel") then
                state.Letter = nil
                bindLetter()
            end
        end)
    ...
    env.WordHelperDiscovery.RecorderAddConnection(
        child:GetPropertyChangedSignal("Name"):Connect(function()
            if child.Name == "Deleted" then
                -- We cannot reliably distinguish a user backspace from Last Letter's
                -- own tile deletion/scroll animation. Fail closed instead of learning
                -- a corrected or truncated misspelling.
                env.WordHelperDiscovery.RecorderAmbiguous = true
                env.WordHelperDiscovery.AcceptanceEventValid = false
                return
            end

            -- Numeric renames are deliberately ignored after initial seeding.
        end)
    )

    -- Ordinary removals are also ignored. They are presentation changes and must
    -- never shorten or rewrite the monotonic internal word history.
end

env.WordHelperDiscovery.RecorderSnapshotCurrent = function(frame, now)
    local container = frame and frame:FindFirstChild("CurrentWord")
    ...
    env.WordHelperDiscovery.RecorderSlots = {}
    env.WordHelperDiscovery.RecorderMaxIndex = 0
    env.WordHelperDiscovery.RecorderBuffer = ""
    env.WordHelperDiscovery.RecorderAmbiguous = false
    env.WordHelperDiscovery.RecorderReady = true
    env.WordHelperDiscovery.RecorderSeeding = true

    local indexed = {}
    for _, child in ipairs(container:GetChildren()) do
        local numericIndex = tonumber(child.Name)
        local letter = child:FindFirstChild("Letter")
        if numericIndex and child:IsA("GuiObject")
            and letter and letter:IsA("TextLabel") then
            indexed[#indexed + 1] = {
                Index = numericIndex,
                Value = letter.Text
            }
    ...
    for _, entry in ipairs(indexed) do
        env.WordHelperDiscovery.RecorderSetSlot(entry.Index, entry.Value, now)
    end
    env.WordHelperDiscovery.RecorderSeeding = false

    -- The initial visible numeric indices must start at 1 and be contiguous.
    -- If we attach halfway through an already-truncated word, skip that turn.
    local maxIndex = tonumber(env.WordHelperDiscovery.RecorderMaxIndex or 0) or 0
    if maxIndex > 0 then
        for index = 1, maxIndex do
            if not env.WordHelperDiscovery.RecorderSlots[index] then
                env.WordHelperDiscovery.RecorderAmbiguous = true
                break
            end
        end
...
env.WordHelperDiscovery.RecorderBeginTurn = function(frame, typeText, now)
    ...
    env.WordHelperDiscovery.RecorderSeeding = false
    env.WordHelperDiscovery.RecorderResetAt = now or tick()
    env.WordHelperDiscovery.RecorderReadyAt = (now or tick()) + 0.16
...
env.WordHelperDiscovery.ObserveFrame = function(detectedWord, censored, typeText, now, frame)
    ...
    if not env.WordHelperDiscovery.RecorderAttach(frame, typeText, now) then
        return
    end

    -- After a turn transition, wait a fraction of a second for Last Letter to
    -- finish replacing the previous accepted-word tiles, then seed the recorder
    -- from the new turn's actual visible starting letters.
    if env.WordHelperDiscovery.RecorderReady ~= true
        and now >= (tonumber(env.WordHelperDiscovery.RecorderReadyAt or 0) or 0) then
        env.WordHelperDiscovery.RecorderSnapshotCurrent(frame, now)
    end
    ...
    elseif typeText ~= env.WordHelperDiscovery.LastTypeText then
        -- The prompt moving to another player is the final acceptance confirmation.
        local pulseAt = tonumber(env.WordHelperDiscovery.AcceptancePulseAt or 0) or 0
        ...
        if pulseAt > 0
            ...
            env.WordHelperDiscovery.ConsiderAccepted(
                candidate,
                "event-driven tile recorder + WPM + turn change"
            )
        end
    ...
    -- WPM is the acceptance pulse. We arm the *internal event buffer* rather than
    -- whatever happens to be visible in CurrentWord at this frame.
    ...
    if wpmText ~= env.WordHelperDiscovery.LastWPMText then
        ...
        if amount and amount > 0
            ...
            env.WordHelperDiscovery.AcceptanceEventBuffer =
                tostring(env.WordHelperDiscovery.RecorderBuffer or "")
