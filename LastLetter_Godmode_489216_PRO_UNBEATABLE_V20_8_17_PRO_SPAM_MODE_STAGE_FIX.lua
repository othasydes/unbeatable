-- V20.8.17: Pro Spam Mode (F9) + corrected 5-turn stage tracker + punctuation-aware server UsedWords/prefixes
...
-- V20.8.17 PRO SPAM MODE (F9)
-- Uses the user's editable Godmode literal-ending priority inside Pro Unbeatable.
-- A spam is live only while the FULL Pro reply universe still has an unused reply.
-- If no live configured spam matches the current candidate pool, normal deterministic
-- Pro Unbeatable ranking is used unchanged.
env.WordHelperUnbeatable.ProSpamMode = env.WordHelperUnbeatable.ProSpamMode or false

env.WordHelperUnbeatable.GetProSpamPriorityIndex = function(word)
    if not env.WordHelperUnbeatable.ProSpamMode then return nil end
    word = tostring(word or ""):lower()
    ...
    for index, category in ipairs(Config.GodmodePriority or {}) do
        if category ~= "__TRAP__"
            ...
            and category ~= ""
            and #word >= #category
            and word:sub(-#category) == category then
...
env.WordHelperUnbeatable.SelectTopProCandidates = function(candidateList)
    ...
    local function better(a, b)
        ...
        -- V20.8.17 Pro Spam Mode: any LIVE configured spam outranks non-spam
        -- candidates, and earlier Godmode spam priorities outrank later ones.
        -- Once both candidates are in the same spam row, the existing Pro comparator
        -- below still chooses lowest live response count and then longest forced reply.
        if env.WordHelperUnbeatable.ProSpamMode then
            local spamA = env.WordHelperUnbeatable.GetProSpamPriorityIndex(a)
            local spamB = env.WordHelperUnbeatable.GetProSpamPriorityIndex(b)
            if spamA ~= spamB then
                if spamA == nil then return false end
                if spamB == nil then return true end
                return spamA < spamB
            end
        end
