...
Title.Text = "<font color=\"rgb(45,255,112)\">qoox\'s</font> wordhelper <font color=\"rgb(105,166,121)\">// V20.9</font>"
Title.RichText = true
...
UpdateList = function(detectedText, requiredLetter)
    ...
    for i = 1, math.max(#displayList, #ButtonCache) do
        ...
        if w then
            ...
            -- V20.8.43 RETURN PREFIX HIGHLIGHT
            -- Display-only: highlights the exact suffix/prefix Last Letter is expected
            -- to return to the opponent. No ranking/search logic is changed.
            env.WordHelperFormatReturnPrefix = env.WordHelperFormatReturnPrefix or function(baseText, word, returnedPrefix)
                local rp = tostring(returnedPrefix or ""):lower()
                if rp == "" or rp == "fallback" or #rp > #word then return baseText end
                if word:sub(-#rp) ~= rp then return baseText end

                local cut = #baseText
                -- Rebuild the word itself so the returned suffix is unmistakable.
                local head = word:sub(1, #word - #rp)
                local returnRGB = "45,255,112"
                return "<font color=\"rgb(" .. ColorToRGB(THEME.Text) .. ")\">"
                    .. head
                    .. "</font><font color=\"rgb(" .. returnRGB .. ")\"><b>"
                    .. rp:upper()
                    .. "</b></font>"
            end
            ...
            elseif sortMode == "Unbeatable" then
                ...
                displayText = env.WordHelperFormatReturnPrefix(displayText, w, ub.Prefix)
                displayText = displayText
            ...
            elseif sortMode == "Pro Unbeatable" then
                ...
                displayText = env.WordHelperFormatReturnPrefix(displayText, w, pro.Prefix)
                displayText = displayText
                    .. "  <font color=\"rgb(150,150,160)\">["
                    .. detail
                    .. "]</font>"
