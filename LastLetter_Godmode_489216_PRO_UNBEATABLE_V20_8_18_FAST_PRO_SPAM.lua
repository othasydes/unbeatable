-- V20.8.18: FAST Pro Spam Mode (F9) + corrected stage tracker + punctuation-aware server UsedWords/prefixes
...
-- Performance rule:
--   1) evaluate each configured spam's live Pro reply count ONCE per list refresh;
--   2) cheaply scan candidates by suffix;
--   3) if a live spam exists, run expensive Pro analysis ONLY on candidates from
--      the highest-priority available spam;
--   4) if no spam exists, use the normal Pro Unbeatable selector unchanged.
    ...
    if input.KeyCode == Enum.KeyCode.F9 then
        ...
        if StatusText then
            StatusText.Text = env.WordHelperUnbeatable.ProSpamMode
                and "Pro Spam: ON / spam -> lowest replies -> longest forced"
                or "Pro Spam Mode: OFF (F9)"
