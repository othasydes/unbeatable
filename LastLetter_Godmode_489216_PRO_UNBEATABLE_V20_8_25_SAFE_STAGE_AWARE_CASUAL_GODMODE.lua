-- V20.8.25: SAFE stage-aware Casual Godmode; candidate generation untouched
...
GodmodeGetPriorityCategory = function(word)
    ...
    -- SAFE V20.8.25: read the existing stage tracker only. No tracker updates,
    -- no refreshes and no candidate filtering occur inside Godmode scoring.
    local currentStage = 1
    if env and env.WordHelperUnbeatable then
        currentStage = math.clamp(tonumber(env.WordHelperUnbeatable.Stage) or 1, 1, 4)
    end
    ...
    for index, category in ipairs(Config.GodmodePriority) do
        ...
        if category == "__TRAP__" then
            allowed = currentStage >= 2
        elseif category == "__X__" or category == "__LOWEST_ENTRY__" then
            allowed = true
        else
            -- A literal spam cannot be returned before its suffix length is unlocked.
            allowed = type(category) == "string" and #category <= currentStage
        end

        if allowed then
            if category == "__TRAP__" then
                if currentStage == 2 then
                    -- Stage 2 uses ONLY the pairs personally confirmed in Casual.
