...
env.WordHelperDiscovery.RecorderTrackChild = function(child)
    ...
    local state = {
        LastNumericIndex = tonumber(child.Name),
        Letter = nil
    }
    ...
    local function bindLetter()
        ...
        local function pushLetter()
            local numericIndex = tonumber(child.Name)
            if numericIndex then
                state.LastNumericIndex = numericIndex
                env.WordHelperDiscovery.RecorderSetSlot(
                    numericIndex,
                    letter.Text,
                    tick()
                )
        ...
        env.WordHelperDiscovery.RecorderAddConnection(
            letter:GetPropertyChangedSignal("Text"):Connect(pushLetter)
        )
    end
    ...
    env.WordHelperDiscovery.RecorderAddConnection(
        child:GetPropertyChangedSignal("Name"):Connect(function()
            local previousIndex = state.LastNumericIndex
            local newIndex = tonumber(child.Name)

            if child.Name == "Deleted" and previousIndex then
                env.WordHelperDiscovery.RecorderDeleteSlot(previousIndex, tick())
                state.LastNumericIndex = nil
                return
            end

            if newIndex then
                state.LastNumericIndex = newIndex
                bindLetter()
                if state.Letter then
                    env.WordHelperDiscovery.RecorderSetSlot(
                        newIndex,
                        state.Letter.Text,
                        tick()
    ...
    -- Deliberately DO NOT erase RecorderSlots when this child is removed.
    -- Last Letter removes old tiles for long-word truncation and after acceptance.
end
