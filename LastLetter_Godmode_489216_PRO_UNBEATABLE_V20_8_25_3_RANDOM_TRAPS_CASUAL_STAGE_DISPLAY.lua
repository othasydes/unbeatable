-- V20.8.25.3: randomized Casual Stage-2 traps + Casual Godmode stage display; core scoring preserved
    ...
    for _, w in ipairs(candidateList) do
        ...
        if needSpamScan then
            ...
            if matchedIndex then
                ...
                elseif cat.Kind == "ENDING" then
                    -- Literal spam: shortest available word wins.
                    -- Equal-length choices use Pro analysis only as a tie-break.
                    if #w < #current or (#w == #current and better(w, current)) then
                        categoryBest[matchedIndex] = w
                    end
                elseif better(w, current) then
                    categoryBest[matchedIndex] = w
                end
            end
        end
