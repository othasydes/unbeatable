...
env.WordHelperBlacklistTracker.HandleWrongSound = function(source)
    ...
    -- The Wrong sound is global. Capture the currently armed local attempt, but
    -- wait a fraction of a second so packet 26 has time to arrive. We only
    -- blacklist when BOTH systems independently identify the exact same word.
    local pendingWord =
    ...
    task.delay(0.12, function()
        ...
        -- Packet proof must be fresh, must belong to this Enter attempt, and must
        -- name the exact same word. If any of those checks fail, skip automatic
        -- blacklisting rather than risking a valid dictionary word.
        local packetMatches =
        ...
        if not packetMatches then
            env.WordHelperBlacklistTracker.ClearPending()
            if StatusText then
                StatusText.Text = "Rejection ignored - packet did not match: " .. pendingWord
                StatusText.TextColor3 = THEME.Warning
            end
            return
        end
        ...
        local added = env.WordHelperBlacklistTracker.Add(
            pendingWord,
            "confirmed exact packet submission + SoundService.Game.Wrong via "
